...
The sole dependency-mutation exception is the `npm_dependencies.set_package` tool in a trusted engineering session with `build:write`. It accepts one exact semantic version, one supported dependency section, and a nested `package.json` with an existing adjacent `package-lock.json` inside the current session-owned clone. The server runs fixed `npm install --package-lock-only --ignore-scripts --no-audit --no-fund --save-exact` arguments with isolated HOME/cache, refuses existing nested `node_modules` and every project `.npmrc` behavior except the exact tracked `legacy-peer-deps=true` compatibility line, verifies the resulting manifest and lockfile, and restores both files on failure. It never writes the shared root dependency tree. Use it instead of hand-editing lockfiles or widening shell npm authority.
