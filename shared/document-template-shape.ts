...
export function extractTemplateHeadings(templateMarkdown: string): ShapeHeading[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!title) continue;
    // Skip residual section itself if present on a template (should not be required).
...
export function validateArtifactShape(
  ...
  // Detect unlabeled-lead requirement: template has a ## that looks like a date
  // (or any first ##) with body text before the next heading in the template.
  ...
  for (let i = 0; i < templateLines.length; i++) {
    ...
    // Lead is required when the template shows unlabeled body under the date heading
    // via an explicit "unlabeled lead" note, or when the first heading is a date pattern
    // and the following non-heading non-omit line is not only a section instruction.
  ...
  for (const heading of templateHeadings) {
    // Date heading on digest: match any ## that looks like YYYY-MM-DD when template uses that pattern.
