...
-- Three account-owned structures back first-party Mod composition. Additive and
-- replay-safe; resolved composition remains computed, never a source-of-truth table.
-- Runtime convergence also runs via server/mod-schema.ts ensureModPlatformSchema.
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_mod_entitlements_account_mod ON mod_entitlements(account_id, mod_key);
CREATE INDEX IF NOT EXISTS idx_mod_entitlements_scope_owner ON mod_entitlements(scope, owner_user_id, account_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_mod_installations_account_mod ON mod_installations(account_id, mod_key);
CREATE INDEX IF NOT EXISTS idx_mod_installations_scope_owner ON mod_installations(scope, owner_user_id, account_id);
CREATE INDEX IF NOT EXISTS idx_mod_installations_status ON mod_installations(account_id, status);
...
CREATE INDEX IF NOT EXISTS idx_mod_installation_resources_installation ON mod_installation_resources(installation_id, status);
CREATE INDEX IF NOT EXISTS idx_mod_installation_resources_scope_owner ON mod_installation_resources(scope, owner_user_id, account_id);
