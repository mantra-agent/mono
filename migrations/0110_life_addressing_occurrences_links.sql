-- Protocol-owned authored-reference projection and generic explicit assertions.
-- Additive and replay-safe: domain facts and semantic relationships stay in
-- their existing authoritative tables.
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_reference_occurrence_source_owner ON reference_occurrence_sources(owner_user_id, account_id, source_address);
CREATE INDEX IF NOT EXISTS idx_reference_occurrence_sources_scope_owner ON reference_occurrence_sources(scope, owner_user_id, account_id, source_address);
CREATE INDEX IF NOT EXISTS idx_reference_occurrence_sources_replay ON reference_occurrence_sources(owner_user_id, account_id, source_observed_at, source_address);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_reference_occurrence_projection_ordinal ON reference_occurrences(source_projection_id, occurrence_ordinal);
CREATE INDEX IF NOT EXISTS idx_reference_occurrences_source ON reference_occurrences(owner_user_id, account_id, source_address, occurrence_ordinal);
CREATE INDEX IF NOT EXISTS idx_reference_occurrences_target ON reference_occurrences(owner_user_id, account_id, target_address, observed_at);
CREATE INDEX IF NOT EXISTS idx_reference_occurrences_scope_owner ON reference_occurrences(scope, owner_user_id, account_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_address_links_owner_idempotency ON address_links(owner_user_id, account_id, idempotency_key);
CREATE INDEX IF NOT EXISTS idx_address_links_source_active ON address_links(owner_user_id, account_id, source_address, predicate) WHERE lifecycle = 'active';
CREATE INDEX IF NOT EXISTS idx_address_links_target_active ON address_links(owner_user_id, account_id, target_address, predicate) WHERE lifecycle = 'active';
CREATE INDEX IF NOT EXISTS idx_address_links_scope_owner ON address_links(scope, owner_user_id, account_id, lifecycle);
