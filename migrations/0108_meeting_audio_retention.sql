...
CREATE UNIQUE INDEX IF NOT EXISTS meeting_audio_samples_owner_session_source ON meeting_audio_samples(owner_user_id, account_id, session_id, source_key);
CREATE INDEX IF NOT EXISTS meeting_audio_samples_owner_created ON meeting_audio_samples(owner_user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS meeting_audio_samples_expiry ON meeting_audio_samples(status, expires_at);
CREATE INDEX IF NOT EXISTS meeting_audio_samples_session ON meeting_audio_samples(session_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS meeting_audio_evaluations_owner_idempotency ON meeting_audio_evaluations(owner_user_id, account_id, idempotency_key);
CREATE INDEX IF NOT EXISTS meeting_audio_evaluations_sample_created ON meeting_audio_evaluations(sample_id, created_at DESC);
CREATE INDEX IF NOT EXISTS meeting_audio_evaluations_owner_created ON meeting_audio_evaluations(owner_user_id, created_at DESC);
