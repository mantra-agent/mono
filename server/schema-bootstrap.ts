...
async function ensureDocumentStoreDocumentsSchema(pool: { query: (sql: string, params?: unknown[]) => Promise<unknown> }): Promise<void> {
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_document_store_owner_type_id ON document_store_documents(scope, owner_user_id, account_id, document_type, document_id)`);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_document_store_source_row ON document_store_documents(source_table, source_row_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_document_store_scope_owner ON document_store_documents(scope, owner_user_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_document_store_account ON document_store_documents(account_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_document_store_vault ON document_store_documents(vault_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_document_store_type_id ON document_store_documents(document_type, document_id)`);
...
async function ensureMeetingTurnEnrollmentSchema(pool: {
  query: (sql: string, params?: unknown[]) => Promise<{ rows: Array<Record<string, unknown>> }>;
}): Promise<void> {
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_meeting_turn_enrollment_source ON meeting_turn_enrollments(owner_user_id, account_id, session_id, source_turn_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turn_enrollments_due ON meeting_turn_enrollments(status, next_attempt_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turn_enrollments_session ON meeting_turn_enrollments(session_id, created_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turn_enrollments_owner ON meeting_turn_enrollments(owner_user_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turn_enrollments_account ON meeting_turn_enrollments(account_id)`);
}
...
export async function runSchemaBootstrap(
  ...
  // The quarantine ledger and its monotonic trigger remain durable even before
  // physical quarantine is authorized. The retired legacy-memory closure is no
  // longer a bootstrap-owned schema in any environment: excluding it here
  // prevents fresh databases, Brain imports, and unquarantined deployments from
  // recreating tables that ordinary runtime no longer serves.
  const {
  ...
  const retiredLegacyMemoryTables = new Set<string>(
    LEGACY_MEMORY_QUARANTINE_TABLES,
  );
  const activeBaselineTables = baselineTables.filter(
    (table) => !retiredLegacyMemoryTables.has(getTableConfig(table).name),
  );
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_exposure_build_claim ON memory_vnext_exposures(owner_user_id, account_id, context_build_id, claim_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_exposure_claim ON memory_vnext_exposures(claim_id, occurred_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_exposure_owner ON memory_vnext_exposures(scope, owner_user_id, occurred_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_exposure_account ON memory_vnext_exposures(account_id, occurred_at)`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_strength_events (
      ...
      CONSTRAINT memory_vnext_strength_event_type_valid CHECK (event_type IN ('explicit_confirmation', 'decision_use', 'goal_relevance', 'confirmed_recurrence', 'contextual_importance', 'correction'))
    )
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_strength_event_replay ON memory_vnext_strength_events(owner_user_id, account_id, event_key)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_strength_event_claim ON memory_vnext_strength_events(claim_id, occurred_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_strength_event_owner ON memory_vnext_strength_events(scope, owner_user_id, occurred_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_strength_event_account ON memory_vnext_strength_events(account_id, occurred_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_strength_event_type ON memory_vnext_strength_events(event_type, occurred_at)`);
  await pool.query(`COMMENT ON COLUMN memory_vnext_claims.recall_count IS 'Legacy compatibility telemetry only. Excluded from strength, certainty, lifecycle, retirement, deletion, and retrieval ranking.'`);
  await pool.query(`COMMENT ON COLUMN memory_vnext_claims.last_recalled_at IS 'Legacy compatibility telemetry only. New passive context exposure is recorded in memory_vnext_exposures.'`);
  ...
  await pool.query(`COMMENT ON COLUMN memory_vnext_claims.confidence IS 'Extraction-confidence compatibility data only. Claim certainty is derived independently from assessed evidence.'`);
  await pool.query(`COMMENT ON COLUMN memory_vnext_sources.strength IS 'Legacy compatibility field. Source clarity and relationship certainty are independent evidence dimensions.'`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_predictions (
      id SERIAL PRIMARY KEY, replay_key TEXT NOT NULL, input_state_claim_ids INTEGER[] NOT NULL,
      action_claim_id INTEGER NOT NULL, action_mode TEXT NOT NULL, predicted_outcome_claim_id INTEGER,
      predicted_outcome_class TEXT NOT NULL, probability REAL NOT NULL, horizon_seconds INTEGER NOT NULL,
      expected_at TIMESTAMPTZ(6) NOT NULL, transition_path_id INTEGER NOT NULL,
      causal_claim_link_ids INTEGER[] NOT NULL DEFAULT '{}', transition_snapshot JSONB NOT NULL,
      generation_context JSONB NOT NULL, producer_method TEXT NOT NULL, derivation_version TEXT NOT NULL,
      model_version TEXT, generated_at TIMESTAMPTZ(6) NOT NULL,
      scope TEXT NOT NULL DEFAULT 'user', owner_user_id TEXT, account_id TEXT, created_by_user_id TEXT,
      created_at TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_prediction_replay ON memory_vnext_predictions(owner_user_id, account_id, replay_key)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_prediction_owner_expected ON memory_vnext_predictions(scope, owner_user_id, expected_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_prediction_account_expected ON memory_vnext_predictions(account_id, expected_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_prediction_action ON memory_vnext_predictions(action_claim_id, generated_at)`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_prediction_resolutions (
      ...
      replay_key TEXT NOT NULL, outcome TEXT NOT NULL, observed_outcome_claim_id INTEGER,
      evidence_source_ref_ids INTEGER[] NOT NULL DEFAULT '{}', evidence_snapshot JSONB NOT NULL,
      actual_value REAL, brier_score REAL, log_score REAL, scoring_rule_version TEXT NOT NULL,
      resolution_method TEXT NOT NULL, derivation_version TEXT NOT NULL, supersedes_resolution_id INTEGER,
      resolved_at TIMESTAMPTZ(6) NOT NULL, scope TEXT NOT NULL DEFAULT 'user', owner_user_id TEXT,
      account_id TEXT, created_by_user_id TEXT, created_at TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_prediction_resolution_replay ON memory_vnext_prediction_resolutions(owner_user_id, account_id, replay_key)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_prediction_resolution_prediction ON memory_vnext_prediction_resolutions(prediction_id, resolved_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_prediction_resolution_owner ON memory_vnext_prediction_resolutions(scope, owner_user_id, resolved_at)`);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_relationship_certainty_events (
      ...
      claim_link_id INTEGER NOT NULL, replay_key TEXT NOT NULL, previous_certainty REAL NOT NULL,
      delta REAL NOT NULL, resulting_certainty REAL NOT NULL, rule_version TEXT NOT NULL,
      provenance JSONB NOT NULL, occurred_at TIMESTAMPTZ(6) NOT NULL,
      scope TEXT NOT NULL DEFAULT 'user', owner_user_id TEXT, account_id TEXT, created_by_user_id TEXT,
      created_at TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_relationship_certainty_event_replay ON memory_vnext_relationship_certainty_events(owner_user_id, account_id, replay_key)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_relationship_certainty_event_link ON memory_vnext_relationship_certainty_events(claim_link_id, occurred_at)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_relationship_certainty_event_resolution ON memory_vnext_relationship_certainty_events(resolution_id)`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_prediction_runs (
      id SERIAL PRIMARY KEY, run_key TEXT NOT NULL, trigger TEXT NOT NULL, scanned_paths INTEGER NOT NULL,
      generated INTEGER NOT NULL, abstained INTEGER NOT NULL, resolved INTEGER NOT NULL, scored INTEGER NOT NULL,
      certainty_updates INTEGER NOT NULL, abstention_reasons JSONB NOT NULL DEFAULT '{}'::jsonb,
      producer_method TEXT NOT NULL, derivation_version TEXT NOT NULL, started_at TIMESTAMPTZ(6) NOT NULL,
      completed_at TIMESTAMPTZ(6) NOT NULL, scope TEXT NOT NULL DEFAULT 'user', owner_user_id TEXT,
      account_id TEXT, created_by_user_id TEXT, created_at TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_prediction_run_key ON memory_vnext_prediction_runs(owner_user_id, account_id, run_key)`);
  ...
  for (const table of [
    ...
    await pool.query(`CREATE TRIGGER ${trigger} BEFORE UPDATE OR DELETE ON ${table} FOR EACH ROW EXECUTE FUNCTION reject_memory_vnext_prediction_ledger_mutation()`);
  }
  await pool.query(`COMMENT ON TABLE memory_vnext_predictions IS 'Immutable shadow forecasts recorded before outcome evidence is visible.'`);
  await pool.query(`COMMENT ON TABLE memory_vnext_prediction_resolutions IS 'Append-only outcome evidence and proper scoring for immutable forecasts.'`);
  await pool.query(`COMMENT ON TABLE memory_vnext_relationship_certainty_events IS 'Append-only provenance for bounded prediction-derived relationship certainty updates.'`);

  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_retrieval_controls (
      id SERIAL PRIMARY KEY, retrieval_mode TEXT NOT NULL DEFAULT 'compatibility', prediction_output_mode TEXT NOT NULL DEFAULT 'shadow',
      reason TEXT NOT NULL DEFAULT 'default_off', scope TEXT NOT NULL DEFAULT 'user', owner_user_id TEXT, account_id TEXT,
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_retrieval_control_owner ON memory_vnext_retrieval_controls(owner_user_id, account_id)`);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS memory_vnext_retrieval_activation_events (
      id SERIAL PRIMARY KEY, replay_key TEXT NOT NULL, previous_mode TEXT NOT NULL, next_mode TEXT NOT NULL, reason TEXT NOT NULL,
      scope TEXT NOT NULL DEFAULT 'user', owner_user_id TEXT, account_id TEXT, created_by_user_id TEXT,
      created_at TIMESTAMPTZ(6) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_activation_event_replay ON memory_vnext_retrieval_activation_events(owner_user_id, account_id, replay_key)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_activation_event_owner ON memory_vnext_retrieval_activation_events(scope, owner_user_id, created_at)`);
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_retrieval_label ON memory_vnext_retrieval_labels(owner_user_id, account_id, context_key, claim_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_retrieval_label_context ON memory_vnext_retrieval_labels(scope, owner_user_id, context_key)`);
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_retrieval_eval_replay ON memory_vnext_retrieval_evaluation_runs(owner_user_id, account_id, replay_key)`);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_retrieval_eval_context ON memory_vnext_retrieval_evaluation_runs(owner_user_id, account_id, context_build_id)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_retrieval_eval_owner ON memory_vnext_retrieval_evaluation_runs(scope, owner_user_id, completed_at)`);
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS uk_memory_vnext_causal_path_review ON memory_vnext_causal_path_reviews(owner_user_id, account_id, prediction_id)`);
  await pool.query(`
  ...
  for (const table of ["memory_vnext_retrieval_activation_events", "memory_vnext_retrieval_evaluation_runs", "memory_vnext_prediction_evaluation_runs"]) {
    const trigger = `trg_${table}_append_only`;
    await pool.query(`DROP TRIGGER IF EXISTS ${trigger} ON ${table}`);
    await pool.query(`CREATE TRIGGER ${trigger} BEFORE UPDATE OR DELETE ON ${table} FOR EACH ROW EXECUTE FUNCTION reject_memory_vnext_evaluation_ledger_mutation()`);
  }
  await pool.query(`COMMENT ON TABLE memory_vnext_retrieval_controls IS 'User-scoped corrected-retrieval authority. Missing row means compatibility and prediction output remains shadow-only.'`);
  await pool.query(`COMMENT ON TABLE memory_vnext_retrieval_evaluation_runs IS 'Immutable bounded dual-retrieval measurements. Focus content is represented only by a hash.'`);
  await pool.query(`COMMENT ON TABLE memory_vnext_prediction_evaluation_runs IS 'Immutable strict-cutoff forecast and baseline evaluation aggregates.'`);
  ...
  const ensureFreshDatabaseFoundation = async () => {
    ...
    // library_pages.memory_entry_id remains an inert historical sidecar until
    // its schema contract is retired separately. It must not recreate an inbound
    // dependency on the retired legacy-memory closure.
    await pool.query(`
  ...
  await heal("connected_accounts columns", async () => {
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_subscription_oauth_transactions_expires ON subscription_oauth_transactions(expires_at)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_subscription_oauth_transactions_provider ON subscription_oauth_transactions(provider)`);
    await pool.query(`
      DO $migration$
      DECLARE
        ray_user_id text;
  ...
  await heal("memory_vnext claim tables", async () => {
    ...
    await pool.query(`ALTER TABLE memory_vnext_claims ADD COLUMN IF NOT EXISTS review_judgment TEXT`);
    await pool.query(`ALTER TABLE memory_vnext_claims ADD COLUMN IF NOT EXISTS review_note TEXT`);
    await pool.query(`ALTER TABLE memory_vnext_claims ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMPTZ`);
    await pool.query(`ALTER TABLE memory_vnext_claims ADD COLUMN IF NOT EXISTS reviewer_user_id TEXT`);
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_memory_vnext_claim_review_judgment ON memory_vnext_claims(review_judgment)`);
    await pool.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
  ...
  await heal("person vault memberships", async () => {
    ...
    const backfill = await pool.query(`
      ...
      SELECT p.id, chosen.vault_id, 'user', p.owner_user_id, p.account_id, p.owner_user_id
      FROM persons p
      CROSS JOIN LATERAL (
    ...
    if (backfill.rowCount) {
      log(`[boot] person vault membership backfill: inserted=${backfill.rowCount}`, "migration");
    }
  });
  ...
  await heal("library_placements table", async () => {
    await pool.query(`
  ...
  await heal("durable compaction operations", async () => {
    await pool.query(`
    ...
    `);
    await pool.query(`ALTER TABLE indexed_content ADD COLUMN IF NOT EXISTS operation_key TEXT`);
    await pool.query(`
    ...
    const reconciled = await reconcileAbandonedCompactions();
    log(
      `[startup] compaction reconciliation inspected=${reconciled.inspected} failed=${reconciled.failed} archivesDeleted=${reconciled.archivesDeleted}`,
      "boot",
    );
  });
  ...
  await heal(
    "migrate code embedding column to 384-dim (local model)",
    async () => {
      const result = await pool.query(
        `
        SELECT a.atttypmod
        ...
        JOIN pg_namespace n ON n.oid = c.relnamespace
        WHERE n.nspname = 'public'
          AND c.relname = 'code_embeddings'
          AND a.attname = 'embedding'
          AND a.atttypmod > 0
        LIMIT 1
        `,
      );
      if (result.rows.length === 0 || result.rows[0].atttypmod === 384) return;
  ...
  await heal("inference payload captures", async () => {
    await pool.query(`
  ...
  await heal("meeting_turns table", async () => {
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turns_ready ON meeting_turns(assembly_status, ready_at)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turns_session ON meeting_turns(session_id, created_at)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turns_owner ON meeting_turns(owner_user_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turns_account ON meeting_turns(account_id)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_meeting_turns_affinity ON meeting_turns(execution_affinity_boot_id) WHERE execution_affinity_boot_id IS NOT NULL`);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_meeting_turns_claimed_session ON meeting_turns(session_id) WHERE execution_status = 'claimed'`);
  });
  ...
  await heal("meeting_recap_distributions table", async () => {
    ...
    await pool.query(`ALTER TABLE meeting_recap_distributions ADD COLUMN IF NOT EXISTS onboarding_token_hash TEXT`);
    ...
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_mrd_onboarding_token_hash_unique ON meeting_recap_distributions(onboarding_token_hash) WHERE onboarding_token_hash IS NOT NULL`);
