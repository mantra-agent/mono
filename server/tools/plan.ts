...
function libraryScopeColumns(libraryPages: typeof import("@shared/models/info").libraryPages) {
  return { scope: libraryPages.scope, ownerUserId: libraryPages.ownerUserId, accountId: libraryPages.accountId, vaultId: libraryPages.vaultId };
}
...
async function getLibraryPage(pageId: string) {
  ...
  const scope = { scope: libraryPages.scope, ownerUserId: libraryPages.ownerUserId, accountId: libraryPages.accountId };
  const byId = await db.select().from(libraryPages).where(combineWithVisibleScope(getCurrentPrincipalOrSystem(), scope, eq(libraryPages.id, pageId)));
  if (byId.length > 0) return byId[0];
  const bySlug = await db.select().from(libraryPages).where(combineWithVisibleScope(getCurrentPrincipalOrSystem(), scope, eq(libraryPages.slug, pageId)));
  return bySlug[0] || null;
}

// ─── Tool Handler ────────────────────────────────────────────────────

export async function handlePlan(args: Record<string, any>): Promise<ToolHandlerResult> {
...
async function handleCreate(args: Record<string, any>): Promise<ToolHandlerResult> {
  const title = args.title as string;
  if (!title) return { result: "Missing required 'title' parameter.", error: true };

  const stepsInput = args.steps as Array<{ title: string; instructions: string }>;
  if (!Array.isArray(stepsInput) || stepsInput.length === 0) {
    return { result: "Missing required 'steps' array. Provide at least one step with title and instructions.", error: true };
  }

  for (let i = 0; i < stepsInput.length; i++) {
    const s = stepsInput[i];
    if (!s.title) return { result: `Step ${i + 1} missing 'title'.`, error: true };
    if (!s.instructions) return { result: `Step ${i + 1} missing 'instructions'.`, error: true };
  }
  ...
  const ownerValues = ownedInsertValues(getCurrentPrincipalOrSystem(), planScopeColumns);
  ...
  try {
    ...
    recordSessionArtifact(sessionId, "library_page", page.slug, { title: page.title, pageId: page.id });
  } catch { /* best effort */ }

  log.log(`[${planId}] Created — ${stepsInput.length} steps, pageId=${page.id}, blocking=${blocking}`);
...
async function handleGet(args: Record<string, any>): Promise<ToolHandlerResult> {
  const planId = args.planId as string;
  if (!planId) return { result: "Missing required 'planId' parameter.", error: true };

  // Try DB first (by plan ID, then by page UUID)
  let plan = await db.select().from(planExecutions).where(visiblePlan(eq(planExecutions.id, planId))).then(r => r[0]);
  if (!plan) plan = await db.select().from(planExecutions).where(visiblePlan(eq(planExecutions.pageId, planId))).then(r => r[0]);

  // Try resolving as a Library page slug → page UUID → planExecutions.pageId
  if (!plan) {
    const resolvedPage = await getLibraryPage(planId);
    if (resolvedPage) {
      plan = await db.select().from(planExecutions).where(visiblePlan(eq(planExecutions.pageId, resolvedPage.id))).then(r => r[0]);
    }
  }
...
async function handleAssociateSession(args: Record<string, any>): Promise<ToolHandlerResult> {
  ...
  if (!sessionId) return { result: "No active session context is available to associate with this plan.", error: true };
...
async function handleList(args: Record<string, any>): Promise<ToolHandlerResult> {
  ...
  const plans = await db.select().from(planExecutions)
    .where(visiblePlan())
...
async function handleExecute(args: Record<string, any>): Promise<ToolHandlerResult> {
  const inputPlanId = args.planId as string;
  if (!inputPlanId) return { result: "Missing required 'planId' parameter.", error: true };
...
async function handleExecuteLegacy(args: Record<string, any>): Promise<ToolHandlerResult> {
  const planId = args.planId as string;
  const page = await getLibraryPage(planId);
  if (!page) return { result: `Plan "${planId}" not found.`, error: true };
  ...
  if (!meta.blocking) {
    executePlan(meta.id, sessionId, planTitle, false).catch(err => {
      log.error(`[${meta.id}] Background execution failed: ${err instanceof Error ? err.message : String(err)}`);
    });
    return { result: `Plan **${planTitle}** started in background (migrated from legacy).` };
  }
...
async function handleUpdateStep(args: Record<string, any>): Promise<ToolHandlerResult> {
  const planId = args.planId as string;
  ...
  const step = await db.select().from(planSteps)
    .where(visiblePlanStep(eq(planSteps.planId, resolvedPlanId)))
    .then(rows => rows.find(r => r.id === stepId));
  if (!step) return { result: `Step "${stepId}" not found in plan.`, error: true };

  const setFields: Record<string, any> = { updatedAt: new Date() };
  const requestedStatus = args.status as string | undefined;
  if (requestedStatus) {
    const validStatuses = new Set(["pending", "completed", "failed", "skipped", "blocked", "needs_review"]);
  ...
  await db.update(planSteps).set(setFields)
    .where(writablePlanStep(and(eq(planSteps.planId, resolvedPlanId), eq(planSteps.id, stepId))));

  // Check if plan is now done
  const allSteps = await db.select().from(planSteps).where(visiblePlanStep(eq(planSteps.planId, resolvedPlanId)));
  ...
  if (isPlanDone(allSteps)) {
    ...
    await db.update(planExecutions).set({ status: newStatus, updatedAt: new Date() })
      .where(writablePlan(eq(planExecutions.id, resolvedPlanId)));
    autoCompleted = true;
  }

  const latestPlanForRefresh = await db.select().from(planExecutions).where(visiblePlan(eq(planExecutions.id, resolvedPlanId))).then(r => r[0]);
  await refreshPlanPage(latestPlanForRefresh || plan, page);

  return { result: `Step "${step.title}" updated: status=${args.status || step.status}${args.outcome ? `, outcome="${(args.outcome as string).slice(0, 100)}"` : ""}${autoCompleted ? ". ✅ Plan auto-completed (all steps done)." : ""}` };
}

async function handleEdit(args: Record<string, any>): Promise<ToolHandlerResult> {
  const planId = args.planId as string;
  if (!planId) return { result: "Missing required 'planId' parameter.", error: true };
  ...
  if (args.projectId !== undefined) setPlan.projectId = args.projectId == null ? null : Number(args.projectId);

  const title = typeof args.title === "string" ? args.title.trim() : "";
  if (title) {
    const { libraryPages } = await import("@shared/models/info");
    await db.update(libraryPages)
      .set({ title: title.startsWith("Plan:") ? title : `Plan: ${title}`, updatedAt: new Date() })
  ...
  if (hasPlanFields) {
    await db.update(planExecutions).set(setPlan).where(writablePlan(eq(planExecutions.id, resolvedPlanId)));
  }

  const stepEdits = args.stepEdits as Array<{ stepId: string; title?: string; instructions?: string; status?: string }> | undefined;
  let editedSteps = 0;
  if (Array.isArray(stepEdits)) {
    ...
    for (const edit of stepEdits) {
      ...
      if (typeof edit.title === "string" && edit.title.trim()) setStep.title = edit.title.trim();
      if (typeof edit.instructions === "string") setStep.instructions = edit.instructions;
      if (edit.status) {
        if (!validStatuses.has(edit.status)) {
          return { result: `Invalid status "${edit.status}" for step "${edit.stepId}". Use pending, completed, failed, skipped, blocked, or needs_review.`, error: true };
        }
      ...
      if (Object.keys(setStep).length > 1) {
        await db.update(planSteps)
          .set(setStep)
          .where(writablePlanStep(and(eq(planSteps.planId, resolvedPlanId), eq(planSteps.id, edit.stepId))));
        editedSteps++;
      }
    }
  }

  const latestPlan = await db.select().from(planExecutions).where(visiblePlan(eq(planExecutions.id, resolvedPlanId))).then(r => r[0]);
  if (latestPlan) await refreshPlanPage(latestPlan, page);

  if (!title && !hasPlanFields && editedSteps === 0) {
    return { result: "No plan edits supplied. Use title, blocking, workspace, goalId, projectId, or stepEdits.", error: true };
  }
...
async function handleAddSteps(args: Record<string, any>): Promise<ToolHandlerResult> {
  ...
  const newSteps = args.newSteps as Array<{ title: string; instructions: string }>;
  ...
  const existingSteps = await db.select().from(planSteps)
    .where(visiblePlanStep(eq(planSteps.planId, resolvedPlanId)))
    .orderBy(planSteps.position);
  ...
  if (insertAfterPosition < existingCount - 1) {
    await db.update(planSteps)
      .set({ position: sql`${planSteps.position} + ${newSteps.length}`, updatedAt: new Date() })
      .where(writablePlanStep(and(eq(planSteps.planId, resolvedPlanId), gt(planSteps.position, insertAfterPosition))));
  }
  ...
  log.log(`[${resolvedPlanId}] Added ${newSteps.length} steps (total now ${existingCount + newSteps.length})`);

  return { result: `Added ${newSteps.length} step(s) to plan. Total steps: ${existingCount + newSteps.length}.\n\nNew steps:\n${newSteps.map((s, i) => `  ${existingCount + i + 1}. □ ${s.title}`).join("\n")}` };
}

async function handlePause(args: Record<string, any>): Promise<ToolHandlerResult> {
  const planId = args.planId as string;
  if (!planId) return { result: "Missing required 'planId' parameter.", error: true };
  ...
  if (isExecuting(resolvedPlanId)) {
    ...
    return { result: `Plan pause requested. The current step will complete before pausing.` };
  }
...
async function handleResume(args: Record<string, any>): Promise<ToolHandlerResult> {
  const planId = args.planId as string;
  if (!planId) return { result: "Missing required 'planId' parameter.", error: true };
  ...
  if (!plan.blocking) {
    resumePlan(resolvedPlanId, sessionId, planTitle, false).catch(err => {
      log.error(`[${resolvedPlanId}] Background resume failed: ${err instanceof Error ? err.message : String(err)}`);
    });
