...
class EventBus extends EventEmitter {
  ...
  publish(event: Omit<BusEvent, "id" | "timestamp" | "bootId">): BusEvent {
  ...
  private persistAsync(busEvent: BusEvent): void {
    if (this._persistFn) {
      this._persistFn(busEvent).catch(() => {});
      return;
    }
  ...
  getRecentEvents(limit = 100, filter?: { category?: string; runId?: string; event?: string }): BusEvent[] {
  ...
  getRunEvents(runId: string): BusEvent[] {
    return this.buffer.filter(e => e.runId === runId);
  }

  getActiveRuns(): { runId: string; startedAt: number; events: number; lastEvent: string }[] {
  ...
  clearActiveRun(runId: string, reason = "manual_cleanup"): { cleared: boolean; runId: string; reason: string } {
    const hasRun = this.buffer.some(e => e.runId === runId);
