...
export class AgentTimerHandler implements TimerHandler {
  async execute(timer: Timer, run: TimerRun): Promise<TimerHandlerResult> {
    ...
    const allToolDefs = await filterBuildToolSchemas(getCurrentPrincipalOrSystem(), authorityToolDefs);
        ...
        case "tool_result": {
          ...
          if (event.toolCallId)
            recordToolCallEnd(
              String(event.toolCallId),
              !!event.isError,
              typeof (event as { failureKind?: unknown }).failureKind === "string"
                ? (event as { failureKind?: string }).failureKind
                : null,
            );
          break;
        }
