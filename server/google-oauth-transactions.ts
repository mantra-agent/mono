...
export async function createGoogleOAuthTransaction(
  ...
  if (principal.actorType !== "system" && !principal.visibleVaultIds.includes(input.vaultId)) {
...
export async function consumeGoogleOAuthTransaction(
  token: string,
  principal: Principal,
  provider: OAuthTransactionProvider = "google",
) {
  if (!principal.userId || !principal.accountId) throw new Error("Authenticated user principal required");
  return db.transaction(async (tx) => {
    const [row] = await tx
      ...
      .where(and(
        eq(googleOAuthTransactions.tokenHash, hash(token)),
        eq(googleOAuthTransactions.ownerUserId, principal.userId),
        eq(googleOAuthTransactions.principalAccountId, principal.accountId),
        eq(googleOAuthTransactions.provider, provider),
        isNull(googleOAuthTransactions.consumedAt),
    ...
    const [vault] = await tx
      ...
      .from(vaults)
      .where(and(
        eq(vaults.id, row.vaultId),
        eq(vaults.accountId, row.principalAccountId),
        eq(vaults.isArchived, false),
      ))
      .limit(1);
