...
export async function ensureRegressionSchema(): Promise<void> {
  ...
  `);
  await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS regression_runs_owner_trigger_key ON regression_runs(owner_user_id, account_id, trigger_key)`);
  await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS regression_runs_plan_unique ON regression_runs(plan_id) WHERE plan_id IS NOT NULL`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS regression_runs_due_status ON regression_runs(status, due_at)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS regression_runs_owner_created ON regression_runs(owner_user_id, created_at DESC)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS regression_runs_environment_created ON regression_runs(environment_id, created_at DESC)`);
  ...
  `);
  await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS issue_regression_contracts_owner_issue_key ON issue_regression_contracts(owner_user_id, account_id, issue_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS issue_regression_contracts_issue ON issue_regression_contracts(issue_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS issue_regression_contracts_owner_updated ON issue_regression_contracts(owner_user_id, updated_at DESC)`);
  ...
  `);
  await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS issue_regression_results_run_issue_key ON issue_regression_results(owner_user_id, account_id, regression_run_id, issue_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS issue_regression_results_run_created ON issue_regression_results(regression_run_id, created_at DESC)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS issue_regression_results_issue_created ON issue_regression_results(issue_id, created_at DESC)`);
  await db.execute(sql`
    ...
    $function$
  `);
  await db.execute(sql`DROP TRIGGER IF EXISTS issue_regression_results_append_only ON issue_regression_results`);
  await db.execute(sql`
