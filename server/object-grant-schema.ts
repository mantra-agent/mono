...
export async function ensureObjectGrantSchema(pool: ConnectionPool): Promise<void> {
  ...
  try {
    ...
    await client.query(`
      DO $migration$
      BEGIN
        -- Converge subject_type domain: widening the allowed set never invalidates existing rows,
        -- so drop-and-recreate unconditionally to migrate a pre-existing 2-value constraint to include team.
        ALTER TABLE object_grants DROP CONSTRAINT IF EXISTS object_grants_subject_type_check;
        ALTER TABLE object_grants ADD CONSTRAINT object_grants_subject_type_check CHECK (subject_type IN ('user', 'invited_subject', 'team'));
        ...
        ALTER TABLE object_grants ADD CONSTRAINT object_grants_object_type_check CHECK (object_type IN ('project', 'milestone', 'task', 'library_page', 'vault', 'drive_resource'));
