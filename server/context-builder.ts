...
async function resolveSessionAgenda(request: ContextRequest): Promise<string> {
  ...
  try {
    ...
    return [
      ...
      "",
      "This persisted agenda is the session's sole workflow. Process it strictly from top to bottom. The first OPEN item below is the only item you may work on now; keep the exchange natural, but steer it back to that item whenever it drifts. Do not start, preview, ask questions for, or perform tools for any later item.",
      "Each item description states its explicit completion condition. Agenda maintenance is part of the work: an item is not finished until that condition is achieved and its state is persisted. The moment the current item's condition becomes true—and before any prose or action for the next item—call `session.complete_agenda_item` with its exact ID and a discrete resolution. Never batch agenda updates at the end of the session.",
      ...
      "- Skipped: use only when the user explicitly chooses not to do the current item; persist it before moving to the next open item.",
      "- Deferred: use only when an external dependency prevents the current item; persist it before moving to the next open item.",
