...
export async function listAccountBillingSummaries(
  ...
  const rows = await db
    .select()
    .from(accountBilling)
    .where(inArray(accountBilling.accountId, accountIds));
...
async function requirePriceMap(): Promise<Map<BillingPriceKey, string>> {
  ...
  const missing = (["tive_custom", "token_overage"] as BillingPriceKey[]).filter((key) => !map.get(key));
  if (missing.length > 0) {
    throw new StripeCollectorError(
      "Closed Price map is incomplete",
      "billing_price_map_incomplete",
      503,
    );
  }
  return map;
...
async function ensurePlatformMeter(): Promise<string> {
  const existing = await stripeRequest<{ data?: Array<{ id: string; event_name?: string }> }>(
    "GET",
    "/billing/meters?limit=100",
  );
...
export async function attachAccountBilling(input: {
  ...
  const [account] = await db.select({ id: accounts.id }).from(accounts).where(eq(accounts.id, input.accountId)).limit(1);
  ...
  const licensedKey = PACKAGE_LICENSED_PRICE[input.packageKey];
  const licensedPriceId = priceMap.get(licensedKey);
  const overagePriceId = priceMap.get("token_overage");
  if (!licensedPriceId || !overagePriceId) {
    throw new StripeCollectorError(
      `Required collector Prices are missing for ${input.packageKey}`,
      "billing_price_map_incomplete",
      503,
    );
  }
  ...
  if (!publicUrl) {
    throw new StripeCollectorError("Public base URL is not configured", "billing_public_url_missing", 503);
  }
  ...
  if (!session.url) {
    throw new StripeCollectorError("Checkout Session has no URL", "billing_checkout_url_missing", 502);
  }
...
export async function recordCancelNotice(accountId: string): Promise<AccountBillingSummary> {
  ...
  if (!row.stripeSubscriptionId) {
    throw new StripeCollectorError("Subscription is not active", "billing_subscription_missing", 409);
  }
  ...
  log.info("billing cancel notice recorded", {
    accountBillingId: updated.id,
...
function paymentMethodKindFrom(object: Record<string, unknown>): BillingPaymentMethodKind | undefined {
  const types = object.payment_method_types;
  if (!Array.isArray(types)) return undefined;
  if (types.includes("us_bank_account")) return "us_bank_account";
...
async function loadRowByCustomerOrAccount(object: Record<string, unknown>): Promise<AccountBillingRow | null> {
  ...
  if (subscription) {
    const [row] = await db
      .select()
      .from(accountBilling)
      .where(eq(accountBilling.stripeSubscriptionId, subscription))
      .limit(1);
    if (row) return row;
  }
  return null;
...
export async function processStripeEvent(event: StripeEvent): Promise<"ignored" | "replay" | "processed"> {
  ...
  if (priceIds.some(Boolean)) {
    const priceMap = await requirePriceMap();
    assertKnownPriceIds(priceIds, priceMap);
  }
  ...
  if (event.type === "checkout.session.completed") {
    ...
    const kind = paymentMethodKindFrom(object);
    if (kind) patch.paymentMethodKind = kind;
...
export async function receiveAccountMeterEvent(input: AccountMeterEventInput): Promise<{
  status: "queued" | "delivered" | "error" | "replay";
  code?: string;
}> {
  ...
  const identifier = `api_call:${input.apiCallId}`;
  ...
  try {
    ...
    if (code === "23505") {
      const [existing] = await db
        .select({ status: billingMeterDeliveries.status })
        .from(billingMeterDeliveries)
        .where(eq(billingMeterDeliveries.apiCallId, input.apiCallId))
        .limit(1);
      return { status: existing?.status === "queued" ? "replay" : (existing?.status ?? "replay") };
    }
    throw error;
  }

  void deliveryQueue.enqueueAndWait(() => deliverMeterEvent(deliveryId)).catch((error) => {
    log.error("billing meter drain failed", error instanceof Error ? error : new Error(String(error)), {
      deliveryId,
    });
  });
...
async function deliverMeterEvent(deliveryId: string): Promise<void> {
  ...
  while (attempts < 5) {
    try {
      ...
      log.warn("billing meter delivery retrying", { deliveryId: delivery.id, attempts, code });
    }
  }
}
