...
export const TOOL_DETAILS: Record<string, ToolDetailEntry> = {
  ...
  system: {
    description: "System operations — get system state snapshot, create issues, retrieve runtime logs, and inspect frontend and context performance. Actions: state, create_issue, logs, frontend_performance, context_health.",
  ...
  library: {
    ...
    actions: {
      resolve_parent: { description: "Resolve the vault-aware semantic placement outcome from title, purpose compatibility context, pageContext, contentSummary, and tags using the vault Index.", requiredParams: ["purpose"], optionalParams: ["title", "pageContext", "contentSummary", "tags"] },
      ...
      create_library_page: { description: "Create a new library page. Creates through the canonical vault-aware save lifecycle. Purpose is compatibility context only; explicit parent is allowed when human-selected; ambiguous automatic placement is returned as review/lint metadata.", requiredParams: ["title"], optionalParams: ["plainTextContent", "purpose", "pageContext", "contentSummary", "parentId", "tags", "status", "surface", "surfaceDurationHours", "surfaceReason", "surfaceSection"] },
