...
export const TOOL_DETAILS: Record<string, ToolDetailEntry> = {
  ...
  memory: {
    ...
    actions: {
      ...
      search_claims: { description: "Search vNext claims from memory_vnext_claims, with legacy memory_entries compatibility only when not filtered to vNext lifecycle. Filter by claimType (state/cause/action), entity links, lifecycle stage, dates, and storage.", optionalParams: ["claimType", "hasEntityLinks", "entityId", "createdAfter", "createdBefore", "integrationStage", "lifecycleStage", "storage", "limit", "offset"] },
  ...
  tasks: {
    ...
    example: 'Create: { "action": "create", "title": "Review proposal", "description": "Read through and provide feedback" }\nComplete: { "action": "complete", "title": "Review proposal" }\nUpdate: { "action": "update", "title": "Review proposal", "priority": "high" }',
    actions: {
      create: { description: "Create a new task. Supports time estimates (estimateLow/estimateHigh in hours) and deadline (YYYY-MM-DD). When both estimates are set, effort is auto-calculated.", requiredParams: ["title", "description"], optionalParams: ["status", "priority", "impact", "effort", "owner", "requiresReview", "projectId", "milestoneId", "tags", "deliverable", "acceptanceCriteria", "context", "output", "estimateLow", "estimateHigh", "deadline", "tokenEstimate"] },
      ...
      update: { description: "Update a task's properties — priority, status, owner, title, description, tags, time estimates, deadline, etc.", optionalParams: ["taskId", "title", "newTitle", "description", "priority", "status", "impact", "effort", "owner", "requiresReview", "projectId", "milestoneId", "tags", "deliverable", "acceptanceCriteria", "context", "output", "estimateLow", "estimateHigh", "deadline", "tokenEstimate"] },
    },
  },
  finance: {
  ...
  meetings: {
    ...
    actions: {
      ...
      set_metadata: { description: "Classify a calendar event by type and optionally link People records from attendee emails. Event types: focus_block, exercise, meeting, planning, admin, personal.", requiredParams: ["googleEventId", "accountId", "calendarId", "eventType"], optionalParams: ["notes", "attendeeEmails"] },
  ...
  priorities: {
    ...
    whenToUse: "Only for check-in artifact linking: set_review, set_daily_plan, get_daily_artifacts, set_weekly_reflection, set_weekly_plan, set_monthly_plan, set_monthly_reflection, set_quarterly_plan, set_quarterly_reflection. Use the goals tool for all goal CRUD operations (create, update, delete, status changes).",
    example: 'Link weekly reflection: { "action": "set_weekly_reflection", "libraryPageId": "page-uuid" }',
    actions: {
      ...
      get_daily_artifacts: { description: "Get the brief and review Library page links for a given date.", optionalParams: ["date"] },
