...
export const TOOL_DETAILS: Record<string, ToolDetailEntry> = {
  ...
  memory: {
    ...
    actions: {
      ...
      search_claims: { description: "Search vNEXT claims from memory_vnext_claims only. Filter by claimType (state/cause/action), entity links, lifecycle stage, dates, and storage. Set includeReviewedRetired for Digest Memory membership (retired+reviewed rows stay visible).", optionalParams: ["claimType", "hasEntityLinks", "entityId", "createdAfter", "createdBefore", "lifecycleStage", "includeReviewedRetired", "storage", "limit", "offset"] },
      review_claim: { description: "Stamp an explicit human review on a vNext claim (useful | incorrect | needs_clarification). incorrect retires through retireClaim. clearReview=true clears the stamp. Does not auto-run from Digest generation.", requiredParams: ["id"], optionalParams: ["judgment", "note", "clearReview"] },
