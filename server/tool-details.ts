...
export const TOOL_DETAILS: Record<string, ToolDetailEntry> = {
  ...
  tasks: {
    ...
    actions: {
      create: { description: "Create a new task. milestoneId is required; if the right milestone is unclear, ask Ray where it belongs before creating. The tool automatically appends the current source session as @session:id when available. Supports time estimates (estimateLow/estimateHigh in hours) and deadline (YYYY-MM-DD). When both estimates are set, effort is auto-calculated.", requiredParams: ["title", "description", "milestoneId"], optionalParams: ["status", "priority", "impact", "effort", "owner", "requiresReview", "projectId", "milestoneId", "tags", "deliverable", "acceptanceCriteria", "context", "output", "estimateLow", "estimateHigh", "deadline", "tokenEstimate"] },
