...
async function walkExpressionPlan(
  ...
  if (plan.type === "metric") {
    if (visited.has(plan.metricId)) {
      return { ok: false, series: unavailableSeries(host, "Equation contains a cycle.") };
    }
  ...
  const left = await walkExpressionPlan(host, plan.left, range, visited);
  if (!left.ok) return left;
  const right = await walkExpressionPlan(host, plan.right, range, visited);
...
async function handleExpression(
  ...
  const walked = await expressionVisit.run(visited, () => walkExpressionPlan(metric, plan, range, visited));
