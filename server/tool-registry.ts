...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  meeting_bot: {
    description: "Send the Mantra Agent meeting bot into a live meeting via Recall.ai. Actions: join (bot joins a Zoom/Google Meet call and streams a live attributed transcript into a meeting session — pass a meeting 'url', or omit it to auto-resolve the current/next calendar event that has a meeting link), status (bot/meeting state for a meeting session), diagnostics (recent inbound Recall webhook delivery outcomes, including rejected signatures), recap (prepare/resurface recap review drafts for eligible attendees), draft_recap (use the same recap distribution pipeline and create an unsent owner-preview draft when the meeting has no eligible recipients; returned @email_draft objects have the full Gmail update_draft editing contract), leave (bot exits the call). The bot is listen-only and appears in the room as 'Mantra Agent'. Requires the Recall.ai integration to be configured in Settings → Integrations (never ask for the API key in chat).",
    category: "calendar",
    parameters: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["join", "status", "diagnostics", "recap", "draft_recap", "leave"], description: "Action to perform" },
        ...
        sessionId: { type: "string", description: "Meeting session ID (for status/leave/recap/draft_recap)." },
