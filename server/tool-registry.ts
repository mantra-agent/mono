...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  memory: {
    ...
    parameters: {
      ...
      properties: {
        ...
        clarity: { type: "number", description: "How explicitly the source expresses the claim, 0–1 (for add_source)" },
        relationshipCertainty: { type: "number", description: "Certainty of this source-to-claim relationship, 0–1 (for add_source); separate from claim certainty" },
        sourceObservedAt: { type: "string", description: "ISO source observation time (for add_source)" },
  ...
  business: {
    description: "Manage the business layer through the canonical principal- and Vault-scoped storage boundary. Three separate object groups: Business Plans (compose a thematic goal + initiative projects + KPI references), KPIs (target + bear/on-track/bull bands over a metric), and Metrics (the raw measured series and its samples). A KPI requires an existing metricId — create the metric first. Plan add_kpi/remove_kpi only attach or detach an existing KPI to a plan; create_kpi/delete_kpi create or destroy the KPI object. Outputs include canonical @business_plan:id, @kpi:id, and @metric:id references.",
    category: "strategy",
    parameters: {
      type: "object",
      properties: {
        action: {
          ...
          enum: [
            ...
            "list_metrics", "get_metric", "create_metric", "update_metric", "delete_metric", "list_samples", "record_sample",
          ],
          description: "Action to perform. Plan actions operate on the plan id; KPI actions use kpiId; Metric actions use metricId.",
        },
        ...
        goalId: { type: "string", description: "Thematic Goal ID (for plan create/set_thematic_goal)" },
        projectId: { type: "number", description: "Initiative Project ID (for add/remove_initiative)" },
        kpiId: { type: "string", description: "KPI ID (for plan add_kpi/remove_kpi and KPI get_kpi/update_kpi/delete_kpi)" },
        metricId: { type: "string", description: "Metric ID (required for create_kpi and all metric get/update/delete/list_samples/record_sample actions)" },
        vaultId: { type: "string", description: "Visible live Vault ID (for plan create/assign_vault)" },
        query: { type: "string", description: "Search filter (for list_kpis/list_metrics)" },
        ...
        standingObjectiveKey: { type: "string", enum: ["trust-security", "reliability-performance", "customer-health", "revenue-runway", "delivery-economics", "product-release", "founder-team", "corporate-stewardship"], description: "Standing operating objective this KPI owns, 1:1 per vault (create/update_kpi)" },
        value: { type: "number", description: "Numeric sample value (record_sample)" },
  ...
  people: {
    ...
    parameters: {
      ...
      properties: {
        ...
        content: { type: "string", description: "Supporting untimed context, evidence, history, or source detail (for add_note/update_note). Use quickSummary for the concise current profile." },
        ...
        email: { type: "string", description: "Primary email (for create/update)" },
        cabinetLevel: { type: "string", enum: ["agent", "user", "family", "cabinet", "community", "network"], description: "Relationship tier (for create)" },
        quickSummary: { type: "string", description: "Concise current profile shown in the UI (for create/update). Prefer this for synthesized research or a compact description of who the person is." },
        ...
        relation: { type: "string", description: "Personal/family relationship (for create/update). One of the predefined values (Mother, Father, Spouse, Cousin, etc.)." },
        professionalRelations: { type: "array", items: { type: "string" }, description: "Professional relationships (for create/update). Choose from: Partner, Investor, Advisor, Colleague, Employee, Vendor (Agent cannot set Customer)." },
        companyId: { type: "string", description: "Canonical Company ID to link this person to (for create/update). Prefer this over the free-text company name so the profile renders a Company reference chip." },
        tags: { type: "array", items: { type: "string", description: "A tag label" }, description: "Cross-cutting tags (for create/update)." },
        notes: { type: "string", description: "Supporting untimed context, evidence, history, or source detail (for create). Do not use as the profile summary." },
  ...
  work: {
    ...
    parameters: {
      ...
      properties: {
        ...
        taskStatus: { type: "string", enum: ["on_hold", "ready", "active", "done"], description: "Task status filter (for list_tasks)" },
        limit: { type: "number", description: "Task page size (for list_tasks, default 25, max 100)" },
        offset: { type: "number", description: "Task pagination offset (for list_tasks, default 0, max 10000)" },
      },
      required: ["action"],
    },
  },
  tasks: {
    ...
    parameters: {
      ...
      properties: {
        ...
        clearFields: { type: "array", items: { type: "string" }, description: "Fields to explicitly clear (set to null). Allowed: description, assigneeSubjectType + assigneeSubjectId together, deadline. Project and milestone placement cannot be cleared. (for update)" },
  ...
  issues: {
    ...
    parameters: {
      ...
      properties: {
        ...
        limit: { type: "number", description: "Page size (for list, default 100, max 500)" },
  ...
  git: {
    ...
    parameters: {
      ...
      properties: {
        ...
        pr_number: { type: "number", description: "PR number (for checkout or merge_pr)" },
  ...
  decisions: {
    ...
    parameters: {
      ...
      properties: {
        action: {
          type: "string",
          enum: ["list", "get", "create", "update", "delete", "lock", "reopen", "add_update", "edit_update", "delete_update", "add_link", "remove_link", "record_judgment"],
        ...
        description: { type: "string", description: "Short decision description (create/update/record_judgment)" },
        ...
        reasoning: { type: "string", description: "Why this judgment was made (record_judgment)" },
        ownerPersonRole: { type: "string", enum: ["self", "partner"], description: "Judgment owner role: self=agent, partner=user (record_judgment)" },
        principleRevisionIds: { type: "array", items: { type: "string" }, description: "Current principle revision IDs that governed this judgment (record_judgment)" },
        sourceSessionId: { type: "string", description: "Source session for replay-safe judgment provenance (record_judgment)" },
        sourceToolCallId: { type: "string", description: "Source tool call for replay-safe judgment provenance (record_judgment)" },
        triggeredByAddress: { type: "string", description: "Canonical address that triggered the judgment, e.g. @question:sessionId:toolCallId (record_judgment)" },
        answerPayload: { type: "object", description: "Structured answer payload for the judgment (record_judgment)" },
      },
      required: ["action"],
    },
  },
  ...
  session: {
    ...
    parameters: {
      ...
      properties: {
        ...
        itemId: { type: "string", description: "Exact stable agenda item ID returned by list_agenda/get_agenda. Required for update_agenda_item and the complete/skip/defer helpers; never guess it." },
  ...
  plan: {
    ...
    parameters: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["create", "get", "associate_session", "unlink_session", "list", "reconcile_library", "execute", "update_step", "edit", "add_steps", "pause", "review", "resume"], description: "Action to perform" },
        ...
        vaultId: { type: "string", description: "Owning Vault for the execution Plan page (for create; defaults to the active Vault). The page is filed under that Vault's canonical Plans folder." },
        stepId: { type: "string", description: "Step ID within the plan (for update_step/review)" },
        decision: { type: "string", enum: ["approve", "request_changes", "retry", "stop"], description: "Human review decision (for review). The call is accepted only from a later interactive human turn after the gate opened." },
        reason: { type: "string", description: "Bounded review reason or requested changes (for review; required for request_changes)." },
        ...
        outcome: { type: "string", description: "Step outcome summary (for update_step). For needs_review this is the scannable ask shown as the review card headline: one or two plain sentences naming exactly what the human must decide or do. Keep it short; put context in reviewDetail. Use canonical reference chips (@page:id, @pr:repo/number, @person:id) instead of pasting URLs or IDs." },
        reviewDetail: { type: "string", description: "Optional supporting detail for a needs_review gate (for update_step). The fuller instructions, context, or evidence the human needs, rendered as a readable body beneath the headline. Prefer short lines or a few bullet-style steps over one dense paragraph, and use canonical reference chips (@page:id, @pr:repo/number) for any objects." },
        newSteps: { type: "array", items: { type: "object", properties: { title: { type: "string" }, instructions: { type: "string" }, persona: { type: "string", description: "Selectable persona for this mission." } }, required: ["title", "instructions", "persona"] }, description: "New steps to add (for add_steps), each with its mission persona" },
        ...
        limit: { type: "number", description: "Page size (for list: default 20, max 100; reconcile_library: default 200, max 500)" },
        offset: { type: "number", description: "Pagination offset (for list and reconcile_library, default 0)" },
        mode: { type: "string", enum: ["preview", "apply"], description: "Reconciliation mode (for reconcile_library; defaults to preview). Apply moves only Plan-registry joined pages and retires only proven-empty duplicate Plans containers." },
      },
      required: ["action"],
    },
  },
