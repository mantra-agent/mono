      ...
      properties: {
        action: {
          ...
          enum: [
            ...
            "list_hiring_slots", "get_hiring_plan", "create_hiring_slot", "approve_hiring_role", "update_hiring_slot", "cancel_hiring_slot", "remove_hiring_role", "get_model", "set_assumption", "link_assumption_kpi", "clear_assumption_kpi", "get_budget", "add_budget_department", "rename_budget_department", "delete_budget_department", "add_budget_category", "rename_budget_category", "delete_budget_category", "add_budget_line_item", "rename_budget_line_item", "delete_budget_line_item", "set_budget_monthly_amount",
          ],
          description: "Action. Plan actions operate on the plan id; KPI actions use kpiId; Metric actions use metricId; Business-entity, Model, and Budget actions use businessId.",
        },
        ...
        businessId: { type: "string", description: "Business entity ID (required for Business-entity actions, get_model, Budget actions, and create_metric; optional filter for list_metrics and reassignment for update_metric)" },
        ...
        assumptionKey: { type: "string", description: "Financial Model assumption field to write or link a KPI to (set_assumption, link_assumption_kpi, clear_assumption_kpi), e.g. newAccountsPerExternalMeeting, startingAccounts, quarterOneNewAccounts, annualAccountChurnPct." },
  ...
  decisions: {
    ...
    parameters: {
      ...
      properties: {
        ...
        content: { type: "string", description: "Update entry text (add_update / edit_update)" },
  ...
  skills: {
    ...
    parameters: {
      ...
      properties: {
        ...
        category: { type: "string", description: "Skill category (list filter, or create/update)" },
        ...
        activity: { type: "string", description: "Activity type (create/update)" },
  ...
  platforms: {
    ...
    parameters: {
      ...
      properties: {
        ...
        status: { type: "string", description: "Status for platform/product create/update (active|paused|archived), or Feature update_feature (ready|in_progress|needs_review). Feature create always starts ready." },
        ...
        projectId: { type: "string", description: "Railway project ID (save_hosting_binding)" },
        ...
        // Features (Build backlog items on a Product)
        featureId: { type: "string", description: "Feature UUID (get_feature/update_feature/archive_feature/delete_feature/link_feature_kpi/unlink_feature_kpi). Alias: id as string." },
        productId: { type: "number", description: "Product ID for list_features filter, create_feature, or update_feature product reassignment" },
        ownerPersonId: { type: "string", description: "Owner Person ID for create_feature/update_feature" },
        summary: { type: "string", description: "Feature summary (create_feature required; update_feature optional)" },
        stage: { type: "string", enum: ["idea", "spec", "develop", "test", "calibrate", "maintain", "deprecate"], description: "Feature stage (create defaults idea; stage change resets status to ready)" },
        specPageId: { type: "string", description: "Optional Library page ID for the Feature spec (create_feature/update_feature)" },
        search: { type: "string", description: "Optional summary search for list_features" },
        kpiAddress: { type: "string", description: "Canonical KPI address for link_feature_kpi (e.g. @kpi:...)" },
