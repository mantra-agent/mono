...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  business: {
    ...
    parameters: {
      ...
      properties: {
        ...
        assumptionKey: { type: "string", description: "Financial Model mix/volume field to write or link a KPI to (set_assumption, link_assumption_kpi, clear_assumption_kpi), e.g. newAccountsPerExternalMeeting, startingAccounts, costPerMql, mqlToSqlPct, sqlToClosedWonPct, annualAccountChurnPct, maxEntrySharePct, maxPlusEntrySharePct, factoryPlusEntrySharePct. quarterOneNewAccounts is retired. Package prices, includes, extras, and markup are rejected — use update_package / update_extras." },
  ...
  routers: {
    description: "Manage named LLM Routers (exclusive model-connector pools) and Account assignment. Distinct from the diagnostic `router` tool. list/get require system:read; create/add_connector/update_connector/move_connector/set_account_router require system:write (Account assignment also needs users:write).",
    category: "system",
    sideEffectDefault: 2,
    sideEffectActions: { list: 0, get: 0 },
    parameters: {
      ...
      properties: {
        action: {
          ...
          description: "Action. list/get are reads; create makes a named Router; add_connector creates an empty model connector on a Router (no secret inherit); update_connector patches status/priorityPinned/tierMappings on any model connector id; move_connector reparents onto a Router UUID; set_account_router assigns an Account's router_id.",
        },
        id: { type: "string", description: "Router UUID (get)" },
        name: { type: "string", description: "Router display name (create)" },
        routerId: { type: "string", description: "Router UUID (add_connector destination; move_connector/set_account_router destination). Required real UUID — never null." },
        kind: {
          type: "string",
  ...
  platforms: {
    ...
    parameters: {
      ...
      properties: {
        ...
        featureId: { type: "string", description: "Feature UUID for get_feature, update_feature, archive_feature, delete_feature, link_feature_kpi, unlink_feature_kpi, list_feature_sessions, list_feature_history, play_feature, fast_forward_feature, pause_feature, stop_feature" },
