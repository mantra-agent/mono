  ...
  skills: {
    ...
    parameters: {
      ...
      properties: {
        ...
        checklist: { type: "array", items: { type: "object", properties: { check: { type: "string" }, weight: { type: "number" }, kind: { type: "string", enum: ["judgment", "tool_invoked"], description: "Evaluation kind. Default 'judgment' (LLM-scored). 'tool_invoked' is deterministic: passes iff the named tool, and optional exact action, has a successful invocation in the run." }, tool: { type: "string", description: "Tool name for kind 'tool_invoked'. Validated against the tool registry at write time." }, action: { type: "string", description: "Optional exact action required for kind 'tool_invoked'. Validated against the named tool's action enum." } }, required: ["check"] }, description: "Structured quality checklist for scoring (for create/update). The checklist is the single quality spec: judgment items are LLM-evaluated; 'tool_invoked' items are evaluated in code and gate terminal status — a failed one marks the run and its launching timer 'degraded'." },
