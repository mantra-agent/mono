...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  business: {
    description: "Manage the business layer through the canonical principal- and Vault-scoped storage boundary. Object groups: Businesses (the first-class Business entity with Definition identity + Values/Vision/Mission pages, resolved to Vaults through business-vault memberships like People), Business Plans (compose a thematic goal + initiative projects + KPI references), KPIs (target + bear/on-track/bull bands over a metric), and Metrics (the raw measured series and its samples). A KPI requires an existing metricId — create the metric first. Plan add_kpi/remove_kpi only attach or detach an existing KPI to a plan; create_kpi/delete_kpi create or destroy the KPI object. Business-entity actions (list_businesses/get_business/create_business/update_business/archive_business and add_business_vault/remove_business_vault/set_business_vaults) manage the Business and its Vault memberships. Outputs include canonical @business_plan:id, @kpi:id, and @metric:id references.",
    category: "strategy",
    parameters: {
      type: "object",
      properties: {
        action: {
          ...
          description: "Action. Plan actions operate on the plan id; KPI actions use kpiId; Metric actions use metricId; Business-entity actions use businessId.",
        },
  ...
  session: {
    description: "Session metadata, agenda, lifecycle, attention, and tree messaging. Prefer list_agenda + complete/skip/defer; never guess item IDs. Children do not inherit agendas. Children inherit the spawner's execution authority; delegation=engineering additionally selects the Engineer persona when the parent has trusted engineering + build:write. Every child uses its own clone.",
    category: "communication",
    ...
    parameters: {
      ...
      properties: {
        ...
        delegation: { type: "string", enum: ["conversation", "engineering"], description: "Child persona mode for spawn_child. Use engineering to select Engineer for implementation, debugging, build, deploy, or Git-writing missions. Execution authority is inherited from the spawner after server validation; defaults to conversation persona mode." },
  ...
  workflows: {
    ...
    parameters: {
      ...
      properties: {
        ...
        title: { type: "string", description: "Workflow run title (create_run) or artifact title (attach_artifact)" },
        objective: { type: "string", description: "Workflow objective (create_run). Preserve the user’s requested outcome and constraints verbatim in substance; do not prescribe architecture, widen scope, or add adjacent improvements." },
