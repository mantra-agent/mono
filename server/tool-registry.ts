...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  system: {
    description: "System operations — get system state, list/fetch/update/create Issues, retrieve runtime logs, check budget, view current-process events, active runs, connected accounts, and tool stats. Actions: state, list_issues, get_issue, update_issue, create_issue, logs, log_files, budget, frontend_performance, context_health, events, active_runs, clear_active_run, accounts, tool_stats. A full log archive is available in the logs/ directory.",
    category: "system",

    parameters: {
      type: "object",
      properties: {
        action: { type: "string", enum: ["state", "list_issues", "get_issue", "update_issue", "create_issue", "logs", "log_files", "budget", "frontend_performance", "context_health", "events", "active_runs", "clear_active_run", "accounts", "tool_stats"], description: "Action to perform. Use log_files to list available log files; use logs to read a specific log file." },
        id: { type: "string", description: "Issue ID (for get_issue/update_issue)" },
        status: { type: "string", enum: ["open", "in_progress", "in_review", "resolved"], description: "Issue status filter (for list_issues) or new status (for update_issue)" },
        excludeStatus: { type: "string", enum: ["open", "in_progress", "in_review", "resolved"], description: "Issue status to exclude (for list_issues)" },
        note: { type: "string", description: "Concise evidence note (for update_issue)" },
        offset: { type: "number", description: "Pagination offset (for list_issues, default 0)" },
        ...
        limit: { type: "number", description: "Max entries to return (for list_issues/logs/events; default 100, list_issues max 500)" },
