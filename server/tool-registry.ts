...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  business: {
    description: "Manage the business layer through the canonical principal- and Vault-scoped storage boundary. Object groups: Businesses, Financial Model, monthly operating Budgets (Department → Category → Line item), Business Plans, KPIs, and Metrics. get_model returns assumptions plus the full projection matrix (months, period rollups, gates, financing) for a Business. Budget actions require businessId; hierarchy mutations use the stable IDs returned by get_budget, and monthly amounts are integer USD cents. A KPI requires an existing metricId — create the metric first. Plan add_kpi/remove_kpi only attach or detach an existing KPI to a plan; create_kpi/delete_kpi create or destroy the KPI object. Business-entity actions manage the Business and its Vault memberships. Outputs include canonical @business_plan:id, @kpi:id, and @metric:id references.",
    category: "strategy",
    parameters: {
      type: "object",
      properties: {
        action: {
          ...
          enum: [
            ...
            "list_hiring_slots", "get_hiring_plan", "create_hiring_slot", "approve_hiring_role", "update_hiring_slot", "cancel_hiring_slot", "remove_hiring_role", "get_model", "get_budget", "add_budget_department", "rename_budget_department", "delete_budget_department", "add_budget_category", "rename_budget_category", "delete_budget_category", "add_budget_line_item", "rename_budget_line_item", "delete_budget_line_item", "set_budget_monthly_amount",
          ],
