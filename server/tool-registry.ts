...
export const TOOLS: Record<string, ToolMeta> = {
  ...
  business: {
    description: "Manage the business layer through the canonical principal- and Vault-scoped storage boundary. Object groups: Businesses, monthly operating Budgets (Department → Category → Line item), Business Plans, KPIs, and Metrics. Budget actions require businessId; hierarchy mutations use the stable IDs returned by get_budget, and monthly amounts are integer USD cents. A KPI requires an existing metricId — create the metric first. Plan add_kpi/remove_kpi only attach or detach an existing KPI to a plan; create_kpi/delete_kpi create or destroy the KPI object. Business-entity actions manage the Business and its Vault memberships. Outputs include canonical @business_plan:id, @kpi:id, and @metric:id references.",
    category: "strategy",
    parameters: {
      type: "object",
      properties: {
        action: {
          ...
          enum: [
            ...
            "list_hiring_slots", "create_hiring_slot", "update_hiring_slot", "cancel_hiring_slot", "get_budget", "add_budget_department", "rename_budget_department", "delete_budget_department", "add_budget_category", "rename_budget_category", "delete_budget_category", "add_budget_line_item", "rename_budget_line_item", "delete_budget_line_item", "set_budget_monthly_amount",
          ],
          description: "Action. Plan actions operate on the plan id; KPI actions use kpiId; Metric actions use metricId; Business-entity and Budget actions use businessId.",
        },
        ...
        businessId: { type: "string", description: "Business entity ID (required for Business-entity actions, Budget actions, and create_metric; optional filter for list_metrics and reassignment for update_metric)" },
        hiringSlotId: { type: "string", description: "Hiring slot ID (list_hiring_slots returns stable IDs)" },
        roleId: { type: "string", description: "Role ID for create_hiring_slot" },
        approvalMonth: { type: "string", description: "Approval month YYYY-MM" },
