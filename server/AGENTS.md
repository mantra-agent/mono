...
Deterministic tool failures use one shared contract in `tool-failure.ts`. Mutation boundaries classify input/permission failures explicitly; adapters preserve that metadata, and `AgentExecutor` fingerprints the normalized operation, quarantines the exact call after its first failed execution, disables tools, and publishes one authoritative degraded response that preserves completed work. Never infer deterministic input failure from a generic handler error, retry an identical quarantined operation, or duplicate mutation normalization in callers.
