...
Projects, milestones, and tasks each carry a non-null `vault_id` as a container and inheritance anchor. `FileProjectStorage` and `FileTaskStorage` own creation defaults: ordinary work uses the creator principal's active vault, while work created from a meeting tool call receives the meeting session's pinned document vault. Work scope-column definitions must omit `vaultId`; vault visibility never participates in project, milestone, or task authorization. Owner/account scope and explicit object grants are the only work visibility authorities.
...
Exact request inspection is also boundary-owned. `inference-payload-capture.ts` persists the complete secret-free request visible immediately before each provider dispatch, scoped to the current user and bounded to the newest 20 captures per user/account. Direct adapters capture the concrete provider request object. Claude Agent SDK calls capture `query({ prompt, options })`, exclude credentials and opaque runtime instances, and label downstream SDK injection as unobservable. Capture remains fail-open. A failed capture must log a bounded allowlist of the complete nested database error chain plus provider/model/activity/session/attempt and ambient-transaction presence; never log stack traces, query parameters, request payloads, credentials, or arbitrary error fields. Never reconstruct a captured call from current context, tool, or session state.
