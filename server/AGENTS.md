...
- Meeting recap review drafts have one producer: `meeting/distribution.ts`. Manual tool actions reuse that path; a zero-recipient preview may substitute the connected Gmail owner's address only when explicitly requested, and always remains an unsent Gmail draft. Recap drafts are ordinary `email_drafts` rows and retain the full canonical Gmail draft mutation contract; recap provenance must never make From, To, CC, BCC, subject, or body read-only.
