...
- Exact session transcript search is a bounded storage operation, not best-effort discovery. Its shared query and operational probe retain principal/account/Vault scope, 30-day recency, `messageCount > 0`, exact title-or-complete-content substring semantics, newest-first ordering, and limits. Storage timeout or failure must surface as a sanitized tool error; never convert it to an empty success or log SQL, parameters, query text, principal identity, plans, or corpus content. The complete-content trigram GIN index disables `fastupdate` to keep reads independent of pending-list growth; replacements use fresh versioned identities and concurrent build-before-retire.
