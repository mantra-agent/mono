...
Post-build regression checking is an ordinary managed Skill pipeline, not a separate product domain. `system-timer-registry.ts` provisions one user-owned `fireOnNextBuild` Timer for the built-in Regression Skill; `timer-scheduler.ts` compares the current Railway commit with the prior boot, restores each Timer owner's exact Principal, and launches the normal Skill path. Issues in principal-scoped `document_store_documents` remain the sole outcome authority. The Skill pages every unresolved Issue through `issues.list`, inspects each through `issues.get`, uses ordinary separately authorized diagnostics, and may call `issues.resolve` only with a bounded affirmative-evidence note. Uncertain and still-open outcomes remain open and exist only in the Skill report; there is no Regression run/contract/result store, Plan requirement, deployment snapshot, bespoke browser DSL, Workflow/Publish launch adapter, engineering tool, or separate dispatcher.
