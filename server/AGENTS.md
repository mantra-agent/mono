...
- **Budget:** `context-budget.ts` separates the model's hard capacity from Mantra's operating envelope. Durable conversation retention scales to 30% of the model window and caps at 100k estimated tokens; the complete provider request (system context + conversation + current in-run evidence + callable tool schemas) scales to 60% and caps at 128k while reserving configured output capacity. `AgentExecutor` compacts against that operating envelope before every dispatch and fails truthfully if it cannot reduce beneath it; heavy docs render as retrieval references and provider-side silent truncation is not used.
...
- Advertised provider context capacity is truthful capability metadata, not the ordinary session operating budget. Between-turn conversation compaction uses the canonical `getBetweenTurnCompactionThreshold(...)` policy: 60% of the available conversation budget, capped at 100,000 estimated tokens. Provider hard-limit safety continues to use the real model context window; do not couple routine context growth to that maximum or duplicate the 100k policy in callers.
