...
- **Budget:** `context-budget.ts` is the only pressure-policy spine. First derive `hardInputLimit = contextWindow − outputReserve`; between-turn and mid-run stages 1/2/3 are named direct fractions of that usable input envelope. The module resolves absolute altitudes and the server publishes them unchanged to the gauge. Every gate compares the same calibrated full assembled input (messages + tool definitions). Between-turn landing keeps the two newest committed context messages; there is no history keep-budget path. `AgentExecutor` compacts before every dispatch and fails truthfully if it cannot reduce beneath the hard limit. Clients may divide published input and altitudes by `hardInputLimit` for rendering but must never derive policy thresholds; heavy docs render as retrieval references and provider-side silent truncation is not used.
...
- Advertised provider context capacity is truthful capability metadata, while pressure policy uses the usable input envelope. Between-turn conversation compaction uses `getContextPressureThresholds` / `getBetweenTurnCompactionThreshold`: compact when calibrated full next input exceeds `0.3 × hardInputLimit`, where `hardInputLimit = contextWindow − outputReserve`; land at minimum viable live context (archive + summary/capsule; no history keep-budget). The same hard-input denominator governs S1/S2/S3, the provider admission check, and the gauge; do not invent a parallel history-only retention currency.
