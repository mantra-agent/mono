...
- Meeting recap review drafts have one producer: `meeting/distribution.ts`. Manual tool actions reuse that path; a zero-recipient preview may substitute the connected Gmail owner's address only when explicitly requested, and always remains an unsent Gmail draft.
