...
Dashboard interactions and Simple relationship progress are intentionally different projections. Simple counts distinct principal-visible People reached through qualifying channels; Dashboard counts canonical principal-visible outbound `email_messages`, remaining qualifying non-meeting People interaction events, and completed external Calendar occurrences. Outbound email is deduplicated by provider/account/message identity regardless of recipient promotion, People email projection never double-counts it, and Calendar copies deduplicate by the canonical occurrence key. Captured meeting sessions enrich Calendar occurrences but never create an additional Dashboard interaction.
