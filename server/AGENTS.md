...
- `files-api.ts` — Connector-global external-file read boundary (multi-provider); vault-scoped whitelist enforcement. Single authorize path: vault gate → bind/ownership/grant → whitelist → adapter. Large reads download the full provider body (no stage ceiling), AES-GCM envelope-encrypt, and stage into vault-scoped R2 + `indexed_content` with fingerprint operationKey `drive-file:{provider}:{id}:v={md5|modifiedTime}:ct={contentType}:enc={utf8|base64}:enc1`. Read results expose `sourceBytes` / `stagedBytes` / `complete` / `next`. Agent pages via `indexed_content.get` / `read_section` (server decrypts)
