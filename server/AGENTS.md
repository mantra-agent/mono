...
- **Run-quality coupling** — the skill checklist is the single quality-specification surface. Items may declare `kind: "tool_invoked"` with a `tool` name and optional exact `action` (registry-validated at the skills tool write boundary); `skill-scoring.ts` owns the one evaluator: deterministic items are computed in code, judgment items by LLM, merging into one passRate. The runner invokes the same deterministic functions at run terminal — a failure marks the skill run and its launching timer run `degraded` (`tool_coverage_failed`). Async scoring below the skill's `scoreThreshold` reconciles `succeeded`→`degraded` / `success`→`degraded` via guarded fromStatus transitions (`reconcileSkillRunStatus`, `reconcileRunStatusBySession`), emitting `skill.run.degraded`. Downgrades are monotonic. Never introduce a second quality-spec vocabulary (e.g. a separate required-tools list) beside the checklist, and never map skill run status straight to timer success without this gate.
