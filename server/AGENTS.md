...
- **Session persistence and discovery:** `chat-file-storage.ts` serializes one session in process and again with a transaction advisory lock across replicas, then reads/parses/modifies and rewrites the complete JSON document in `document_store_documents.content`. The same transaction deterministically replaces bounded `session_search_segments` plus one `session_search_projections` state row; both are rebuildable derived state with FK-cascade deletion and no independent product mutation path. `session.search` queries segment text and joins the canonical document for principal/account/Vault scope, recency, title, and status; it may use whole-document search only as an explicit rollback or incomplete-backfill fallback. Indexed JSONB metadata supports lists and filters; `session_tree` is a relational side index with best-effort legacy repair.
...
- Session discovery is a bounded storage operation, not best-effort absence. The shared projection query and operational probe retain principal/account/Vault scope by joining `document_store_documents`, 30-day recency, `messageCount > 0`, literal substring semantics over useful bounded segments, newest-first ordering, and limits. Normal search must never return or recheck whole serialized chat content. Storage timeout or failure surfaces as a sanitized tool error; never convert it to an empty success or log SQL, parameters, query text, principal identity, plans, or corpus content. Projection convergence builds a versioned segment-text trigram index concurrently, backfills recent documents in bounded owner-restored transactions, and retains whole-document search only for explicit rollback or incomplete coverage.
