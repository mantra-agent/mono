...
`ensureUserIdentityFoundation(...)` is the canonical replay-safe account activation boundary. Under one user-scoped transaction lock it get-or-creates the existing personal Account, owner membership, profile rows, and one default Personal Vault, then persists and returns authoritative active/visible Vault state. A confirmed creation name may name only rows created in that activation; replay must never rename an existing Account or Vault. Setup, registration, recap claim, login repair, onboarding replay, and boot compatibility repair must converge through this invariant rather than provisioning parallel account/container state. Authenticated workspace onboarding then provisions reserved roots, including Meetings through `ensureMeetingsRoot(...)`, under that returned Vault-pinned principal.
...
Library2 is retired from runtime use. Standard Library routes and tools must not create or maintain Wiki, Index, or Log metadata, run semantic placement, compile pages, execute corpus migrations, or read/write `library_placements`. Existing Vaults, pages, metadata pages, and placement rows remain intact for rollback and historical fidelity. Retired implementation modules may remain only as unreachable compatibility code; they must have no route, tool schema, boot hook, scheduler, or ordinary-save caller. Any future reactivation requires an explicit product decision and must restore one canonical, principal-scoped mutation boundary rather than reintroducing hidden background organization.
...
- **Session persistence:** `chat-file-storage.ts` serializes one session in process and again with a transaction advisory lock across replicas, then reads/parses/modifies and rewrites the complete JSON document in `document_store_documents.content`. Every canonical write advances `durableRevision`; terminal SessionManager events publish that committed revision, and focused HTTP snapshots return metadata plus messages from one document read so clients can fence regressive handoff state. Indexed JSONB metadata supports lists and filters; `session_tree` is a relational side index with best-effort legacy repair.
...
Do not add parallel create helpers that bypass these fields. Do not soften the gate in routes or the tool layer — map validation errors to 400 / `inputFailure`, leave enforcement in storage. Legacy issues may have empty `reproSteps` or null env/build; new creates may not. Regression Skill and bootstrap instructions treat evidence-rich Issues (repro + env/build) as the only actionable queue.
...
`chat-file-storage.ts` → `invalidateSessionsCache(delta?)` publishes `data:sessions_changed` events. When a delta is provided, the event payload includes `{ action: 'created' | 'updated' | 'deleted', sessionId, session? }`. The client applies deltas directly to its cache without refetching. Session Menu REVIEW is composed at the authenticated Session-index route from canonical Session question state plus principal-visible `email_drafts` whose `session_id` is set and status remains `draft`; draft create/send/discard publish the same session-change event without duplicating review state in chat documents.
...
`working-set-projector.ts` is AgentExecutor's single model-facing tool-evidence boundary. Durable transcript state and `indexed_content` preserve exact evidence; the provider request is a disposable bounded projection. An executor-owned tool result remains exact through the first successful post-tool inference. Eligibility for historical projection is ledger-driven (`providerConsumedToolCallIds`, with index `< lastAssistant` fallback). **Every consumed tool result** leaves the exact path as either an archive receipt (`tool-output-artifacts.ts`) or a compact digest; size gates archival only, never eligibility. Never persist a provider receipt/digest/checkpoint in place of exact transcript evidence, mutate saved history to relieve provider pressure, break assistant/tool protocol pairs, or add a second archive/retrieval system.

Active-history budget: after consumed receipts/digests, if projected active-history tokens still exceed `ACTIVE_HISTORY_BUDGET_TOKENS`, checkpoint the oldest eligible **completed** cycle spans (protect current incomplete + `PROTECTED_RECENT_COMPLETED_CYCLES` newest completed). Collapse each span on the provider projection only into one checkpoint message via `summarizeCompactedMessages` (deterministic capsule fallback). Accept a checkpoint only when `tokensSaved >= MATERIAL_CHECKPOINT_REDUCTION_TOKENS`. Phase-1 is provider-view only — no new tables, no durable transcript rewrite required. `working_set_refresh` remains material-reduction only (`MATERIAL_REFRESH_REDUCTION_TOKENS`).

Every `projectWorkingSet` emits attribution telemetry (`exact` / `receipt` / `digest` / `checkpoint` token buckets, cycle counts, receipts/digests/checkpoints applied). AgentExecutor publishes the full telemetry object on `agent.loop.model_request` and `agent.working_set_projection`.

Receipt swap is ledger-driven: `RunIterationContext.providerConsumedToolCallIds` is the durable discriminant for "provider has seen the exact result." AgentExecutor marks IDs (a) after a successful inference that included them and (b) immediately on a material `working_set_refresh` boundary so the next `projectWorkingSet` can emit receipts without waiting for another temporal guard. Every pre-inference `projectWorkingSet` call must receive that set. Cycle counters (`currentCycleToolResultTokens`, `currentCycleRefreshReductionTokens`) measure only the unreceipted window and reset on material refresh or successful consumption; they never substitute for the ledger. Material refresh fires when either the cycle tool-result budget is hit or accumulated `refreshReductionTokens` reaches `MATERIAL_REFRESH_REDUCTION_TOKENS`.
...
- `ACTIVE_HISTORY_BUDGET_TOKENS` default `28000` — soft wall for provider-facing active history after receipts/digests.
- `PROTECTED_RECENT_COMPLETED_CYCLES` default `1` — newest completed cycles kept exact (plus the incomplete current cycle).
- `MATERIAL_CHECKPOINT_REDUCTION_TOKENS` default `2000` (falls back to material-refresh floor) — minimum tokens a cycle checkpoint must save to be accepted.
- `ACTIVE_HISTORY_CHECKPOINT_DEADLINE_MS` default `10000` — soft wall for checkpoint summarizer work inside one projection.
...
- The parent plan executor is the sole owner of managed step completion, failure, retry, and attempt finalization. Plan-spawned children may report only `blocked` or `needs_review`. Ending the child session (`status: saved`) is necessary lifecycle evidence, not completion. The executor completes a step only after observing at least one successful tool invocation (`status: "done"`) in the child transcript via `extractSuccessfulToolInvocations` from `skill-scoring.ts` — the same structural gate skills use. Hollow saved sessions fail the step and pause the plan on both the live monitor path and crash-recovery path; never call `completePlanStepAttempt` without that evidence.
...
- Advertised provider context capacity is truthful capability metadata, while pressure policy uses the usable input envelope. Between-turn history reset uses `getContextPressureThresholds` / `getBetweenTurnCompactionThreshold`: reset when calibrated full next input exceeds `0.3 × hardInputLimit`, where `hardInputLimit = contextWindow − outputReserve`; land at minimum viable live context (archive + summary/capsule; no history keep-budget). The same hard-input denominator governs mid-turn tool soft trim, mid-turn history hard trim, mid-turn history reset, provider admission, and the gauge. Mid-turn history reset is exactly `1.0 × hardInputLimit`; do not add a hidden buffer or parallel history-only retention currency.
