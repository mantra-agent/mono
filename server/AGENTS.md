...
- **Contact identity:** Person `contactInfo` is canonical; `person_emails` is its derived lookup index. `PeopleStorage.savePerson()` must update both atomically and reject an email indexed to another Person rather than reassign it. Agent email updates replace only the labeled primary email and preserve secondary contacts. Person merge may snapshot and repoint linked Opportunity Person IDs, but must select only the reference columns it owns so unrelated Opportunity schema drift cannot block identity repair.
