...
- **News event dedup identity is fingerprint-first** — pre-insert scan candidates have no `signal_items.id`, so every deterministic or model-facing recent-event comparison must cross `excludeSignalFromRecentDigest(...)` with the canonical URL fingerprint before overlap scoring or duplicate-ID selection. A persisted row ID is defense in depth when available. Never remove distinct fingerprints merely because their thesis/topics overlap; cross-article same-event matching is intentional.
- **News curation handoff accounting is set-based** — the exact fingerprints handed to `curate`, the unique buffered decisions, and the unique applied decisions are the canonical bounded sets. Derive eligible, applied, skipped, and unmatched from intersections/differences and emit one outcome discriminant. Decision-field assignments and signal-row upserts are operation counts, never denominators for unique-decision telemetry. Contradictions must log the bounded fingerprint evidence and never render as success.
