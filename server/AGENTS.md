...
- `files-index-service.ts` / `files-index-schema.ts` — Files semantic index policy (`off|self|recursive`), materialized `indexed_file_sources`, reconciliation-run stubs; routes on `/api/files/index/*`. Toggle is the sole mutation boundary; extraction stays in vNext queue.
