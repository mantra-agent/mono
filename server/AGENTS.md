...
- Meeting recognition preserves one canonical ingest path. Calendar metadata supplies only the default for newly discovered sources; `MeetingSessionMeta.audioSourcePolicies` owns runtime policy by stable transport audio-source key. Recall participant streams and native browser microphones both terminate at the provider-neutral STT boundary and then `ingestMeetingEvent(...)`; ordinary sources use Scribe, explicitly shared sources use Deepgram diarization, and both emit stable speaker keys before transcript persistence. `MeetingSessionMeta.transport` isolates native lifecycle from Recall controls. Display labels and Person identities never own audio-source policy.
