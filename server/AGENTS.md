...
- Tool failure observability is producer-owned: classified `ToolFailure.kind` and stable `ToolFailure.code` must travel through `ExecutorRunResult.toolCalls` into both interactive and autonomous persisted tool-call metadata, including duplicate toolCallId merge/rebuild paths. Reliability readers may retain legacy extraction fallbacks, but must not infer a missing code when the producer supplied one. Tool-output pressure telemetry is principal-scoped by both account and user before aggregation; reports expose tool/action counts and size/archival metrics, never arguments or result bodies. The `tool_output_admissions` table is boot-ensured by `server/migrations/ensure-tool-output-admissions.ts` (SQL migration 0116 alone is not sufficient on live). Database failures at this boundary log only bounded PostgreSQL code/type diagnostics, never query parameters or principal identifiers.
