...
- Meeting recognition preserves one canonical ingest path. Calendar metadata snapshots the per-event speaker policy into `MeetingSessionMeta`; ordinary Recall participant streams use Scribe, explicitly selected shared-room streams use Deepgram diarization, and both emit stable speaker keys before transcript persistence. Display labels never own identity.
