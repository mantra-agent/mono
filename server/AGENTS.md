...
- **Memory:** active context retrieval runs semantic and recency seeds in parallel, then at most two sequential graph hops with frontier/result limits before one batched provenance read. Settled-source polling is bounded enqueue discovery only. Every source change increments `source_version`; one deterministic runtime run is enqueued per `(queue_id, source_version)`, and the kernel attempt fence is the sole execution claim. `processing` is legacy compatibility state only and never authorizes native work.
- **Timers/autonomy:** non-migrated timers retain one process-local serial promise with a 12 s between-run cooldown. Weekly Ideas bypasses that owner and only enqueues its deterministic runtime run; the kernel worker owns execution. Scheduler enumeration is cross-account and database-backed, and selected run transitions use guarded updates. Autonomous admission defaults to 20 total / 14 request / 6 background slots, which bounds executor runs rather than SQL statements. Concurrent sessions multiply context reads, API-call audit writes, document checkpoints, tool operations, artifact indexing, and post-run ingestion.
...
- **Compatibility serialized queue** — non-migrated timers run one at a time with a 12s stagger. Weekly Ideas only enqueues a deterministic native runtime run and bypasses this execution owner.
