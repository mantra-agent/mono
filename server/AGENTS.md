...
- **Budget:** `context-budget.ts` is the only pressure-policy spine. Between-turn fire and mid-run stages 1/2/3 are named direct fractions of `contextWindow`; the module resolves absolute altitudes and the server publishes them unchanged to the gauge. Every gate compares the same calibrated full assembled input (messages + tool definitions). `hardInputLimit = contextWindow − outputReserve` is provider admission only, never a ladder parent. Between-turn landing keeps the two newest committed context messages; there is no history keep-budget path. `AgentExecutor` compacts before every dispatch and fails truthfully if it cannot reduce beneath the hard cliff. Clients may divide published altitudes by the window for rendering but must never derive policy thresholds; heavy docs render as retrieval references and provider-side silent truncation is not used.
...
- Advertised provider context capacity is truthful capability metadata, not the ordinary session operating budget. Between-turn conversation compaction uses `getBetweenTurnFireThreshold` / `getBetweenTurnCompactionThreshold`: **fire when full next input > 0.3 × contextWindow**; **land at minimum viable live context** (archive + summary/capsule; no history keep-budget). Provider hard-limit safety continues to use the real model context window; do not invent a parallel history-only retention currency.
