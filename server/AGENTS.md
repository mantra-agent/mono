...
- **Compatibility serialized queue** — non-migrated timers still run one at a time with a 12s stagger. Weekly Ideas bypasses it and only enqueues its deterministic native runtime run; the kernel worker owns execution.
