...
- **Budget:** `context-budget.ts` is a single spine: `hardInputLimit = contextWindow − outputReserve`, `operatingInputLimit = hardInputLimit`, mid-run `compactionTarget = 0.92 × operating`. Between-turn **fires** when full next input (system + history + tools + …) exceeds `getBetweenTurnFireThreshold` = **0.3 × contextWindow**, then lands at **minimum viable live context** (no history keep-budget). `AgentExecutor` compacts against the operating envelope before every dispatch and fails truthfully if it cannot reduce beneath it; heavy docs render as retrieval references and provider-side silent truncation is not used.
