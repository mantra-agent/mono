...
- Exact session transcript search is a bounded storage operation, not best-effort discovery. `document_store_documents` remains canonical; `session_search_segments` is its FK-owned, rebuildable projection of bounded title, agenda, message, and tool text. Every ordinary session write replaces the projection inside the same chat advisory transaction, and search joins the canonical document for principal/account/Vault scope, 30-day recency, `messageCount > 0`, newest-first ordering, and limits. The segment trigram GIN index disables `fastupdate`; backfill is bounded and reads switch only after coverage is complete, with the whole-document query retained temporarily as rollback. Storage timeout or failure must surface as a sanitized tool error; never convert it to an empty success or log SQL, parameters, query text, principal identity, plans, or corpus content.
