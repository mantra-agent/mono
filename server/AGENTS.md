...
- Exact session transcript search is a bounded storage operation, not best-effort discovery. `document_store_documents` remains canonical; `session_search_segments` is its FK-owned, rebuildable projection of bounded title, agenda, message, and tool text. Streaming draft checkpoints never request search work. Each eligible settled canonical write appends one owner-scoped, revision-keyed request to the repository-wide `transactional_outbox` in the same chat transaction; the post-ready projection worker restores the exact owner principal, acquires the chat advisory lock, re-reads the writable canonical document, rejects stale revisions, and replaces the projection after commit through bounded insert statements and bounded worker concurrency. Search joins the canonical document for principal/account/Vault scope, 30-day recency, `messageCount > 0`, newest-first ordering, and limits. The segment trigram GIN index disables `fastupdate`; backfill reuses the same durable reconciliation path and reads switch only after revision-complete coverage, with the whole-document query retained temporarily as rollback. Projection failure may delay search convergence but must never delay or roll back canonical Session persistence; retries and logs remain bounded and content-free. Storage timeout or failure must surface as a sanitized tool error; never convert it to an empty success or log SQL, parameters, query text, principal identity, plans, or corpus content.
