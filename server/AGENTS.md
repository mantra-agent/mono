...
`regression_runs`, `issue_regression_contracts`, and `issue_regression_results` are the principal-owned PostgreSQL authority for deployment regression. `server/regression/regression-admission.ts` is the sole launch boundary: manual admission always creates a fresh run, while deployment admission deduplicates by environment + deployment + revision across Workflow acceptance and post-ready runtime reconciliation. Launch is always available to Ray or Agent and is not a security gate. `server/regression/regression-service.ts` remains the sole ordinary run/result mutation boundary: candidates are snapshotted once per run, one Plan may be associated, and one append-only result may exist per run + issue. Issue notes never store regression history, and results never change Issue status. `server/regression/browser-executor.ts` accepts only the semantic V1 DSL and executes through `withTargetBoundBrowserPage(...)`, which establishes any short-lived Platform-binding session server-side and confines HTTP/WebSocket traffic to the immutable acceptance-target origin from the run snapshot; callers never supply a host, selector, script, cookie, token, or secret. These are execution-integrity controls, not reasons to restrict when a run may start.
