...
const building: ModDefinition = {
  key: "building",
  version: "1.0.0",
  name: "Building",
  description: "The current Build product area: Platforms, Design, Database, Issues, Prompts, and build workflows.",
  outcome: {
    ...
    activationSignals: ["building.route.build", "building.route.platforms"],
  },
  experience: { primaryObjectKind: "platform", primaryActionId: "building.action.platforms", rootSurfaceKey: "build" },
  compatibility: { minimumCoreVersion: MIN_CORE },
  requiresCore: ["automation", "integration-custody", "ui-composition"],
  recommendsMods: ["planning"],
  contributions: {
    clientRoutes: [
      clientRoute("building.route.build", "/build", "build", { requiredPermissions: ["build:read"] }),
      clientRoute("building.route.database", "/database", "database", { requiredPermissions: ["build:read"] }),
      clientRoute("building.route.design", "/design", "design", { requiredPermissions: ["build:read"] }),
      clientRoute("building.route.platforms", "/platforms", "platforms", { requiredPermissions: ["build:read"] }),
      clientRoute("building.route.platform-environment-detail", "/platforms/environments/:id", "platform-environment-detail", { requiredPermissions: ["build:read"] }),
      clientRoute("building.route.issue-detail", "/issues/:id", "issue-detail", { requiredPermissions: ["build:read"] }),
      clientRoute("building.route.debug-toast", "/dev/toast", "debug-toast", { requiredPermissions: ["build:read"] }),
    ],
    navigation: [
      nav("building.nav.platforms", "Build", "Platforms", "Boxes", "building.route.platforms", 1, { requiredPermissions: ["build:read"] }),
      nav("building.nav.design", "Build", "Design", "Palette", "building.route.design", 2, { requiredPermissions: ["build:read"] }),
      nav("building.nav.toast", "Build", "Toast", "MessagesSquare", "building.route.debug-toast", 3, { requiredPermissions: ["build:read"] }),
      nav("building.nav.database", "Build", "Database", "DatabaseZap", "building.route.database", 4, { requiredPermissions: ["build:read"] }),
      nav("building.nav.issues", "Build", "Issues", "Hammer", "building.route.build", 5, { requiredPermissions: ["build:read"] }),
      nav("building.nav.prompts", "System", "Prompts", "FileText", "building.route.build", 5, { requiredPermissions: ["build:read"] }),
    ],
    integrations: [
      integration("building.integration.github", "github", "available", ["source"]),
      integration("building.integration.railway", "railway", "available", ["hosting"]),
      integration("building.integration.cloudflare", "cloudflare", "available", ["hosting", "dns"]),
      integration("building.integration.expo", "expo", "available", ["mobile-build"]),
      integration("building.integration.sentry", "sentry", "available", ["error-tracking"]),
    ],
    workflows: [workflowRef("building.workflow.build-v1", "build-v1")],
    actions: actionsForOwner("building"),
  },
};
