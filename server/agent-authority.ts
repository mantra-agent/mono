...
export function getShellToolContractDescription(): string {
  ...
  return [
    ...
    "Deny: newlines, backticks, `$(...)`, bare `&`, `<`/`>` redirects, `~`, unquoted or double-quoted variable expansion. Single-quote inert `import type { Principal } from "./principal";
import { getCurrentPrincipal } from "./principal-context";
import { getSideEffectTier } from "./autonomy-tiers";
import { principalHasPermission, type Permission } from "./permissions";
import { isWorkflowStageAction } from "./workflows/stage-capability";
...
const SHELL_DENIAL_GUIDANCE: Readonly<Record<string, string>> = {
  ...
  "forbidden:variable_expansion":
    "Shell variable expansion ($VAR / ${...}) is blocked. Substitute the literal value yourself; when searching source for dollar syntax, put the pattern in single quotes so it stays inert.",
