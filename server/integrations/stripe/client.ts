...
export async function requireStripeSecretKey(): Promise<string> {
  ...
  if (!key) {
    throw new StripeCollectorError("Stripe secret key is not set", "billing_secrets_missing", 503);
  }
...
export function verifyStripeSignature(rawBody: string, header: string | undefined, secret: string): boolean {
  ...
  const signatures = header
    .split(",")
    .filter((piece) => piece.trim().startsWith("v1="))
    .map((piece) => piece.trim().slice(3));
