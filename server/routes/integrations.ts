...
(function () {
  ...
  app.put("/api/connected-accounts/:id/vault", requireAuth, async (req, res) => {
    try {
      const vaultId = String(req.body.vaultId || "");
      if (!vaultId) return res.status(400).json({ error: "vaultId is required" });
      const { assignConnectedAccountVault } = await import("../connected-accounts");
      res.json(await assignConnectedAccountVault(req.params.id, vaultId));
    } catch (error: any) {
      const message = error.message || "Vault assignment failed";
      res.status(message.includes("moves are blocked") ? 409 : 400).json({ error: message });
    }
  });
  ...
  const twilioStatus = async (req: any) => {
    ...
    return {
      ...
      smsWebhookUrl: `${callbackBase}/api/webhooks/twilio/sms`,
      mediaStreamUrl: `${callbackBase.replace(/^http/, "ws")}/ws/twilio-media`,
      runtimeEnvironment: runtime.environmentName,
      servingHost: runtime.servingHost,
      publicUrl: runtime.publicUrl,
    };
