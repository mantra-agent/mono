...
function libraryMemoryFields(page: LibraryPage, content: string) {
  const pageTitle = (page.title || "").trim() || null;
  const pageTags = Array.isArray(page.tags) ? page.tags : [];
  const hasStageOne = Boolean(
    pageTitle &&
      page.summary?.trim() &&
      pageTags.length > 0,
  );
...
export async function upsertLibraryPageMemory(
  ...
  const content = page.plainTextContent || page.title || "";
  const sourceId = page.id;
  log.debug(`[ingest] start source=library sourceId=${sourceId} pageId=${page.id} contentLength=${content.length}`);
  if (!content.trim()) {
    log.debug(`[ingest] skip source=library sourceId=${sourceId} reason=empty_page_content`);
    return;
  }

  try {
    const principal = getCurrentPrincipalOrSystem();
    const { pageTitle, pageTags, hasStageOne, contentHash, stage } = libraryMemoryFields(page, content);
    const existingLibRaw = await db
      .select(memoryEntryLightColumns)
      .from(memoryEntries)
      .where(
        combineWithVisibleScope(
    ...
    const existingLib = existingLibRaw.map((r) =>
      wrapLightEntry(r as Omit<MemoryEntry, "embedding">),
    );

    if (existingLib.length > 0) {
      const existing = existingLib[0];
      ...
      if (contentChanged) {
        await db
          ...
          .set({
            ...
            title: pageTitle,
            oneLiner: hasStageOne ? page.oneLiner || null : null,
            summary: hasStageOne ? page.summary || null : null,
            tags: hasStageOne ? pageTags : [],
            contentHash: hasStageOne ? contentHash : null,
            processedAt: new Date(),
            updatedByUserId: principal.userId ?? undefined,
          })
          .where(
        ...
        log.info(`[ingest] updated_memory_entry source=library sourceId=${sourceId} memoryEntryId=${existing.id} layer=short integrationStage=${stage} hashChanged=true`);
        log.debug(`[ingest] integration_stage_set source=library sourceId=${sourceId} memoryEntryId=${existing.id} stage=${stage}`);
        ...
        await db
          ...
          .where(eq(libraryPages.id, page.id));
        log.debug(`[ingest] memory_entry_writeback source=library sourceId=${sourceId} pageId=${page.id} memoryEntryId=${existing.id}`);
      } else {
        // Content unchanged: only refresh title pointer; preserve existing summary/oneLiner.
        const updates: Record<string, unknown> = { processedAt: new Date(), updatedByUserId: principal.userId ?? undefined };
        if ((existing.title || null) !== pageTitle) updates.title = pageTitle;
        if (hasStageOne) {
          updates.integrationStage = MEMORY_INTEGRATION_STAGE.ENRICHED;
          updates.oneLiner = page.oneLiner || null;
          updates.summary = page.summary || null;
          updates.tags = pageTags;
          updates.contentHash = contentHash;
        }
        if (
          Object.keys(updates).length > 1 ||
          (existing.title || null) !== pageTitle
        ) {
          await db
            .update(memoryEntries)
            .set(updates)
        ...
        log.debug(`[ingest] unchanged_hash source=library sourceId=${sourceId} memoryEntryId=${existing.id} hash=${existing.contentHash || "legacy_content_compare"}`);
        await db
          .update(libraryPages)
          .set({ memoryEntryId: existing.id })
          .where(eq(libraryPages.id, page.id));
        log.debug(`[ingest] memory_entry_writeback source=library sourceId=${sourceId} pageId=${page.id} memoryEntryId=${existing.id}`);
      }
      await ensureLibraryMemorySourceRef(existing.id, page.id);
      scheduleMemoryLinks(existing.id).catch((e) =>
        log.warn(`[ingest] link_schedule_failed source=library sourceId=${sourceId} memoryEntryId=${existing.id} error=${errorMessage(e)}`),
      );
    } else {
      ...
      log.info(`[ingest] created_memory_entry source=library sourceId=${sourceId} memoryEntryId=${entry.id} layer=short integrationStage=${stage}`);
      log.debug(`[ingest] integration_stage_set source=library sourceId=${sourceId} memoryEntryId=${entry.id} stage=${stage}`);
      await db
        .update(libraryPages)
        .set({ memoryEntryId: entry.id })
        .where(eq(libraryPages.id, page.id));
      log.debug(`[ingest] memory_entry_writeback source=library sourceId=${sourceId} pageId=${page.id} memoryEntryId=${entry.id}`);
      await ensureLibraryMemorySourceRef(entry.id, page.id);
      scheduleMemoryLinks(entry.id).catch((e) =>
        log.warn(`[ingest] link_schedule_failed source=library sourceId=${sourceId} memoryEntryId=${entry.id} error=${errorMessage(e)}`),
      );
    }
  } catch (error: unknown) {
    log.error(`[ingest] error source=library sourceId=${sourceId} pageId=${page.id} error=${errorMessage(error)}`);
    throw error;
  }
}
