...
export const QUESTION_TOOL_DESCRIPTION =
  "Ask the user one bounded clarification question as an inline Session Window widget, then stop and wait for their response. Use only when the answer cannot be inferred from available context. Principle-first: before calling this tool, load relevant Principles; if they yield one clear answer, do not ask — record a closed Decision yourself via recordJudgment with ownerPersonRole:\"self\", the governing principleRevisionIds, and reasoning. Every accepted Question answer becomes a closed, provenance-linked Decision automatically (decided_by / governed_by / triggered_by). When the fork is genuine, pass considered principles via the principles field and set allowResponseReasoning when the why matters.";
