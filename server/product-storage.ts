...
import { insertProductContextSchema, insertProductSchema, platformProductEnvironments, platforms, productContextArtifacts, productPlatformAssociations, products } from "@shared/models/platforms";
...
export const productStorage = {
  async list() {
    ...
    const [associations, contextRows] = await Promise.all([
  ...
  async remove(id: number) {
    ...
    const [counts] = await db.select({ platforms: sql<number>`count(distinct ${productPlatformAssociations.id})::int` }).from(productPlatformAssociations).where(eq(productPlatformAssociations.productId, id));
    ...
    const dependencies = { platforms: counts?.platforms ?? 0, environments: environmentCount?.environments ?? 0 };
