...
export const productStorage = {
  ...
  async listContext(productId: number) {
    const principal = requireCurrentPrincipal();
    const [product] = await db.select().from(products).where(combineWithVisibleScope(principal, scopeColumns, eq(products.id, productId))).limit(1);
    if (!product) return undefined;
    const rows = await db.select({
      ...
      libraryPageId: productContextArtifacts.libraryPageId,
      pageTitle: libraryPages.title,
    }).from(productContextArtifacts).innerJoin(libraryPages, eq(productContextArtifacts.libraryPageId, libraryPages.id)).where(and(eq(productContextArtifacts.productId, productId), combineWithVisibleScope(principal, libraryScopeColumns))).orderBy(productContextArtifacts.kind, libraryPages.title);
    return rows.map((row) => ({ id: row.id, productId: row.productId, kind: row.kind, libraryPageId: row.libraryPageId, pageTitle: row.pageTitle || "Untitled" }));
  },
