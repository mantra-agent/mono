...
The source poller is scheduled by `server/index.ts` after 30 seconds and every five minutes. One invocation processes at most 10 settled sources sequentially, extracts chunks sequentially, and retains at most three claims per source. This limits local fan-out, but the queue lease is currently structurally incomplete: `pollSettledSources()` selects pending rows and `markProcessing()` updates each row later without a status guard, transaction, `FOR UPDATE SKIP LOCKED`, or claim token. Two app replicas may process the same source. Content hashes, unique queue identity, and claim dedup make much of the write path replay-safe, but they are recovery/idempotency controls—not mutual exclusion. Do not document this queue as cross-replica claimed until selection and transition become one atomic database operation.

`applyObservation()` is the canonical graph mutation boundary for new ingestion. Source content and embeddings may be prepared outside the transaction; matched/created claims, source evidence, and typed relationships commit through that boundary. Avoid holding database transactions across extraction/model calls. Lifecycle/detail/count APIs contain explicit `Promise.all` fan-out, but each branch must remain bounded; the existing `getCounts()` eight-query fan-out is a diagnostic cost, not a pattern to copy into foreground context assembly.
...
Chat-session discovery uses a bounded rebuildable projection, never whole serialized JSON in the normal read path. `writeConv(...)` atomically persists `document_store_documents` and deterministically replaces `session_search_segments` plus one `session_search_projections` state row inside the same advisory-locked transaction. The projection contains bounded title, agenda, message, redacted tool, resolution, and outcome segments with stable keys; `ON DELETE CASCADE` owns deletion consistency. It has no route, tool, or independent mutation API and is never canonical transcript truth.

`buildProjectionSessionSearchQuery(...)` is the normal shared query and operational-probe contract. It applies literal substring matching to bounded segment text, selects one match per session, joins `document_store_documents` by its internal PK, and applies the canonical principal/account/Vault predicate, 30-day recency, positive `messageCount`, newest-first order, and result limit from that canonical row. Never copy ownership/Vault fields into the projection, authorize on segment identity, join by public `document_id`, parse whole session content for snippets, or issue one follow-up query per result. Runtime search remains inside a transaction-local 2.5 s PostgreSQL statement timeout and fails with a sanitized error rather than empty success.

`document-search-indexes.ts` converges `SESSION_SEARCH_SEGMENT_TEXT_INDEX` concurrently after readiness under one cross-replica advisory lock, then backfills missing/stale recent projections in 25-document batches with a 5,000-document pass ceiling. Discovery enumerates only bounded canonical identity; each rebuild restores the persisted owner principal, row-locks the exact document, commits one projection replacement, and yields between batches. The operational probe runs the exact shared query with one real authorized scope and emits only coarse timing/index/coverage counts. Incomplete coverage or `SESSION_SEARCH_READ_SOURCE=legacy` selects the bounded legacy blob query for rollout/rollback; the legacy path remains structurally expensive and is not ordinary architecture. Existing whole-blob indexes may remain temporarily as rollback assets but are no longer schema intent or maintained by the projection owner.

Projection coverage is a correctness signal, not merely performance telemetry. Source `updated_at`, projection version, content hash, eligible/kept/truncated segment counts, FK cascade, transaction atomicity, and deterministic replay collectively expose or prevent divergence. Diagnostics must never include search text, principals, snippets, plans, or corpus content.
