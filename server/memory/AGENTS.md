...
The source settlement service is scheduled by `server/index.ts` after 30 seconds and every five minutes. One invocation discovers at most 10 settled sources and enqueues one `memory.source.process@1` runtime run per monotonic `(queue_id, source_version)` identity. It performs no extraction and writes no processing claim. The kernel attempt lease is the sole cross-replica execution owner; the handler rechecks source version/runtime linkage before graph mutation and completes the source row through the exact same guard. `processing` remains compatibility read state only, with bounded upgrade repair for rows left by pre-kernel builds.
