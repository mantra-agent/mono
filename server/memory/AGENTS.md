...
The source poller is scheduled by `server/index.ts` after 30 seconds and every five minutes, but it is enqueue discovery only. One invocation considers at most 10 settled pending source versions. Every material source change increments `source_version` and clears its projection; the poller enqueues exactly one `memory.source.process@1` run by `memory-source/{queue_id}/version/{source_version}` and links that run only while the same version remains pending. The runtime attempt lease and fence are the sole execution claim across replicas. Extraction remains sequential inside one handler, retains at most three claims, rechecks the source version immediately before consequential mutation, and completes only through the matching `(queue_id, source_version, runtime_run_id)` projection. Legacy `processing` rows may be reset for historical recovery but no native path writes `processing` or treats it as ownership.
