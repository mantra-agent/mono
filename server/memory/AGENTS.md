...
`semantic-source-adapters.ts` is the only source-content load boundary for the poller. `VNEXT_SOURCE_TYPES` currently includes `session`, `library_page`, and `drive_file`. Adapters are thin: identify → authorize → normalize text → return one envelope. Queue hashing, claim extraction, provenance, retries, and Runtime fencing stay in the shared core. `drive_file` source ids are `drive_resources.id`; content loads only through `FilesApi` (authorize + staged archive/normalized text). Personal graph projection maps `drive_file` source refs to durable `file:<driveResourceId>` nodes under the same principal visibility rules as FilesApi.

Files semantic indexing policy is separate from binds: `file_index_policies` (mode `off|self|recursive` on a bound `drive_resource`), `indexed_file_sources` (materialized discovered files only), and `file_index_reconciliation_runs` (durable folder-run stubs). `files-index-service.ts` is the sole ordinary toggle/status mutation boundary; file enable enqueues `markSourceChanged('drive_file', driveResourceId)`, folder enable/disable enqueues a reconciliation run stub. Coverage is multi-policy with no per-child exclusions in v1 — disable retires only sources not covered elsewhere (reconciler finalizes recursive coverage).
