...
export type RuntimeProcessRole = "web" | "worker" | "combined";

function resolveRuntimeProcessRole(value = process.env.RUNTIME_PROCESS_ROLE): RuntimeProcessRole {
  const role = value?.trim().toLowerCase() || "combined";
  if (role === "web" || role === "worker" || role === "combined") return role;
  throw new Error("RUNTIME_PROCESS_ROLE must be web, worker, or combined");
}

export function runtimeProcessOwnsClaims(role = resolveRuntimeProcessRole()): boolean {
...
export function startRuntimeWorker(): RuntimeProcessRole {
  const role = resolveRuntimeProcessRole();
  if (!runtimeProcessOwnsClaims(role)) {
    log.info("runtime.worker.disabled", { role, reasonCode: "web_enqueue_only" });
    return role;
  }
  if (started) return role;
  started = true;
  tick();
  timer = setInterval(tick, CLAIM_INTERVAL_MS);
  timer.unref();
  log.info("runtime.worker.started", { workerId, role, pools: NATIVE_POOLS, claimBatchLimit: CLAIM_BATCH_LIMIT });
  return role;
}
