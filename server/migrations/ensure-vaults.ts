  ...
  try {
    ...
    const failure =
      err instanceof Error
        ? err
        : new Error(typeof err === "string" && err.trim() ? err : "Failed to ensure vault schema");
    if (!/^[A-Z][A-Z0-9_]{1,48}$/.test((failure as Error & { code?: string }).code ?? "")) {
      (failure as Error & { code?: string }).code = "FAILED_TO_ENSURE_VAULT_SCHEMA";
    }
    log.error("Failed to ensure vault schema", failure);
  }
}
