...
3. **Hold / handshake** — once per write-port, one unflushed `"... "` (`openWritePort` / `writeFirstContentHandshake`). After that, comments only (`: presence_hold`, `: keepalive`). No spoken filler (`One moment.`, `One second.`, `Still on it.`, `Working.`). A second handshake on the same socket is unrepresentable. Handshake is not transcript, not `coalesceBuf`, not `segmentChronology`.
