...
- Turn liveness is a versioned, additive protocol negotiated in the voice start response. Accepted browser transcript evidence is tentative; only the provider custom-LLM callback admits the turn and commits transcript history. Persist browser-heard and callback-admitted timestamps on the owned `voice_session_active` lease, then emit one idempotent `voice_turn_recovery` event when accepted browser evidence has no callback within the bounded window. Recovery must never fabricate a user message or begin model execution.
