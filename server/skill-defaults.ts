...
Surface any errors returned by the cycle explicitly in the Memory section and in the session output. Never silently drop them.`,
  },
  {
    ...
    checklist: [
      ...
      { check: "Useful cadence-specific logic is preserved: daily uses Digest template headings (not Brief), weekly uses Weekly Summary template, monthly synthesizes weekly artifacts, quarterly/annual synthesize lower-cadence artifacts", weight: 3 },
...
1. Call \`templates(action: "resolve", skill: "reflect", key: "daily")\` for daily — bound to \`@template:daily-digest\`, **not** Brief.
2. Call \`templates(action: "resolve", skill: "reflect", key: "weekly")\` for weekly — bound to \`@template:weekly-summary\`.
3. Read the resolved Library shape page. Its headings are the vessel.
4. Write a *different* artifact page against those headings. After write, if a required heading is missing or empty (except omit-if-empty), append \`### Residual\` (daily day entry) or \`## Residual\` (weekly) listing the missing headings, and name them in the skill-run / session note.
...
### Daily
Purpose: Daily Digest closeout (Structured Daily Digest taxonomy). Distinct from Brief.
...
- Resolved \`daily-digest\` template page headings (SSOT for sections).

Write against the template vessel (typical global headings):
- \`## YYYY-MM-DD\`
- unlabeled lead (factual opener before the first ###)
- \`### Moved\`
- \`### Open\` (omit if empty)
- \`### Learning\` (one line; omit only when none)
- \`### Memory\` (always; rows or \`None\`)
- \`### Residual\` only when required headings are missing

Claim-review UX is owned by Structured Daily Digest — do not invent it here.

Save to Library:
- running Daily Digest page identity unchanged when one already exists for the account; otherwise create under the established Digest/journal collection
- title / tags follow existing Digest convention; do not file as Brief
