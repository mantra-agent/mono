...
- The brief should make the next planning/review cycle easier to run.`
  },
  {
    ...
    checklist: [
      { check: "Loaded every unresolved Issue page through system.list_issues", weight: 4, kind: "tool_invoked", tool: "system", action: "list_issues" },
      { check: "Inspected every returned Issue individually through system.get_issue", weight: 4, kind: "tool_invoked", tool: "system", action: "get_issue" },
...
1. Call \`system(action: "list_issues", excludeStatus: "resolved", limit: 500, offset: 0)\`. Follow \`nextOffset\` until \`hasMore\` is false so every unresolved Issue is included.
2. Iterate through every returned Issue in order. For each Issue:
   - call \`system(action: "get_issue", id: issueId)\`;
...
3. Only for \`resolved\`, call \`system(action: "update_issue", id: issueId, status: "resolved", note: "<brief evidence>")\`.
