...
Root `AGENTS.md`, root `CODING.md`, root `SECURITY.md`, and `server/AGENTS.md` are mandatory. This file owns only council and session-tree constraints.
...
`server/council/orchestrator.ts` owns one bounded council run. It creates advocate children through `spawnChildSession`, runs at most five rounds, persists each child as an ordinary scoped Session, and writes the synthesis into the parent through the injected principal-scoped chat boundary. One advocate may degrade; both failing is terminal. Cost and token totals are observability, never execution authority.

Council run identity must flow into child spawn reasons and session keys so retries reuse the intended children without colliding with another council run. Model assignment is fixed per advocate request; changing the model requires a new child/run identity.

## Invariants

- Never mutate Session JSON or `session_tree` with an unscoped raw query.
- Never infer authority from a parent ID, spawn reason, tool text, title, or council role. Principal visibility and direct-relative topology are independent gates.
...
- Preserve bounded ancestry, child enumeration, council rounds, and cross-session chain depth.
- Update this file when the actual composition or authority boundary changes; do not document retired `server/session/*` paths or a nonexistent `sessions` row/message JSONB store.
