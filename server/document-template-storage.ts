...
async function assertPageVisible(principal: Principal, pageId: string): Promise<{ id: string; title: string; plainTextContent: string }> {
  const [page] = await db
    .select({
      id: libraryPages.id,
...
export class DocumentTemplateStorage {
  ...
  async list(query?: string, principal?: Principal): Promise<DocumentTemplate[]> {
    ...
    const search = needle
      ...
      : undefined;
    // Prefer account overlay over global when both exist for the same id.
    const rows = await db
      .select()
    ...
    for (const row of rows) {
      ...
      // Account overlay wins over global.
    ...
    return db.transaction(async (tx) => {
      ...
      if (writable) {
        const [updated] = await tx
          ...
          .set(patch)
          .where(and(eq(documentTemplates.id, normalized), eq(documentTemplates.scope, "user"), eq(documentTemplates.accountId, current.accountId)))
          .returning();
        return updated ? mapTemplate(updated) : null;
      }

      // Overlay: copy global row then apply patch.
  ...
  async bind(skillId: string, key: string, templateId: string, principal?: Principal): Promise<SkillTemplateBinding> {
    ...
    const [skill] = await db
      .select({ id: skills.id, name: skills.name, scope: skills.scope, ownerUserId: skills.ownerUserId, accountId: skills.accountId })
      .from(skills)
      .where(eq(skills.id, skillId))
      .limit(1);
    if (!skill) throw new Error("Skill not found");
    // Skill must be visible: global or owned.
  ...
  async listBindingsForSkill(skillId: string): Promise<SkillTemplateBinding[]> {
    const rows = await db
      ...
      .where(eq(skillTemplateBindings.skillId, skillId));
  ...
  async resolve(
    ...
    try {
      ...
      // Resolve skill by id or name (prefer user shadow, else global).
      let skillId: string | null = null;
      let skillName: string | null = null;
      const [byId] = await db.select({ id: skills.id, name: skills.name }).from(skills).where(eq(skills.id, skillNameOrId)).limit(1);
      if (byId) {
        skillId = byId.id;
        skillName = byId.name;
      } else {
      ...
      const [binding] = await db
        ...
        .limit(1);
      if (!binding) {
        // Fallback: if user shadow has no bind, try global skill of same name.
        if (skillName) {
          const [globalSkill] = await db
            .select({ id: skills.id })
            .from(skills)
