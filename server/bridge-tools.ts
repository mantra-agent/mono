...
export const bridgeHandlers: Record<string, ToolHandler> = {
  ...
  async session(args) {
    ...
    if (action === "spawn_child") {
      ...
      // A child is a delegated continuation of its spawner. Preserve trusted
      // engineering provenance whenever the parent already holds it instead of
      // silently downgrading an omitted/default delegation argument.
      ...
      try {
        ...
        if (spawnResult.reused) {
          ...
          const authorityNote = engineeringDelegation
            ? "Authority inherited from the spawner. This child can use independently authorized engineering tools and must use its own session-scoped clone."
