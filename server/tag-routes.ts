...
export function registerTagRoutes(app: Express) {
  ...
  app.get("/api/tags/co-occurrences", async (req, res) => {
    try {
      const index = await tagService.getIndex(req.principal);
  ...
  app.get("/api/tags/:slug", async (req, res) => {
    try {
      const tag = await tagService.getTag(req.params.slug, req.principal);
  ...
  app.post("/api/tags", async (req, res) => {
    try {
      ...
      res.status(201).json(await tagService.createTag(parsed.data, req.principal));
    } catch (error: unknown) {
  ...
  app.patch("/api/tags/:slug", async (req, res) => {
    try {
      ...
      if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
      const tag = await tagService.updateTag(req.params.slug, parsed.data, req.principal);
      if (!tag) return res.status(404).json({ error: "Tag not found" });
      res.json(tag);
    } catch (error: unknown) {
  ...
  app.delete("/api/tags/:slug", async (req, res) => {
    try {
      const deleted = await tagService.deleteTag(req.params.slug, req.principal);
  ...
  app.post("/api/tags/merge", async (req, res) => {
    try {
      ...
      if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
      const tag = await tagService.mergeTags(parsed.data.sourceSlug, parsed.data.targetSlug, req.principal);
  ...
  app.delete("/api/tags/:slug/assignments/:entityType/:entityId", async (req, res) => {
    try {
      ...
      if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
      await tagService.unassignTag(req.params.slug, parsed.data.entityType, parsed.data.entityId, req.principal);
      res.status(204).send();
    } catch (error: unknown) {
  ...
  app.put("/api/tag-assignments/:entityType/:entityId", async (req, res) => {
    try {
      ...
      if (!parsed.success) return res.status(400).json({ error: parsed.error.message });
      const tags = await tagService.replaceEntityTags(
        parsed.data.entityType,
        parsed.data.entityId,
        parsed.data.entityTitle,
        parsed.data.tags,
