...
## Server Standards Disposition

`server-standards-disposition.json` is the source-owned reviewed/cured/exempted matrix for `server/`, `shared/`, `script/`, `scripts/`, and `migrations/`. `script/server-standards-disposition.ts` validates it against the same classified inventory returned by this boundary: every in-scope file must match exactly one evidence-backed owner entry, ordinary authored source cannot be exempted, and exceptional responsibility classes must remain explicit exemptions. The production build runs this gate immediately after repository classification, so new server-side files cannot enter without a disposition and exceptional artifacts cannot masquerade as ordinary cures.
