...
| Conversations | PostgreSQL incremental message rows + session metadata envelope | `conversation_messages` via `conversation-persistence.ts`; `document_store_documents` via `chat-file-storage.ts` retains session metadata and legacy read input |
...
3. **Conversation persistence** — `conversation_messages` is the principal/account/Vault-scoped, ordered message authority. `chat-file-storage.ts` serializes mutations once, advances one durable revision, and writes stable message/run/turn/attempt identity incrementally; `document_store_documents` retains the session metadata envelope and legacy blob read migration only.
