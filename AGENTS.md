...
Never put durable correctness paths (ACLs, settings, billing, ownership mutations) on the telemetry sink. Their SLOW rows are pool-contention symptoms — fix attribution/caching/query shape, not fire-and-forget durability. STT and other transport callback serialization may use `createSerialAsyncDelivery` directly when the consumer is not a DB telemetry insert. During graceful shutdown the shared telemetry sink drops silently once `areDatabasePoolsClosing()` is true: writing to an ended pool throws, the failure re-enqueues as another aggregate, and the loop floods the provider log-rate limiter and starves shutdown/boot evidence.
...
| `server/sessions/AGENTS.md` | Session tree projection, spawn, and child handoff |
