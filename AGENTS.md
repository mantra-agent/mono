...
| External files (multi-provider) | Google Drive + Mantra object storage (+ Box stub) | `server/files-api.ts` + `server/files-providers.ts` — vault-bound reads only; authorize in FilesApi, transport in adapters; large Drive bodies stage to `indexed_content` with source-fingerprint cache keys |
