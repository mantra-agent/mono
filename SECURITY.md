...
The deterministic boundary is the owner-scoped `person_vault_memberships` join plus `server/person-vault-access.ts` and `PeopleStorage`. Authenticated Person reads require owner/account visibility and an `EXISTS` membership in a currently visible, live Vault owned by the same account. List, detail, bulk lookup, search, query, context, agenda, meeting, Simple surfacing, and Company member/count consumers inherit that producer boundary. Membership replacement is non-empty, validates every Vault against the principal account, and replaces the set in one transaction. New People receive the principal's active Vault. Person merge unions source memberships into the target before cascade deletion. Existing rows backfill from a valid legacy `persons.vault_id`, then the account's default live Vault, then its first live Vault by stable position; unresolved People remain without membership and fail closed.

Threat mapping: STRIDE information disclosure, tampering, and elevation of privilege; OWASP API1/API3/API5; IAM-02, DATA-01, DATA-04, AGENT-03, OBS-01. Owner: Application Security Owner. Closure evidence: `migrations/0091_person_vault_memberships.sql`, `shared/schema.ts`, `server/schema-bootstrap.ts`, `server/person-vault-access.ts`, `server/people-storage.ts`, `server/person-merge-service.ts`, `server/people-routes.ts`, `server/company-storage.ts`, `server/simple/people-surface-state.ts`, and Person details UI. Residual risk: repository policy forbids active authorization exploit testing and tests, so evidence is static architecture/impact review plus the production build. Rollback is the merged PR revert; the additive membership table may remain inert.
