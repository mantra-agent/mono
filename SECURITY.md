...
<!-- 2026-07-23 Library vault-identity repair delta: A02/S2 private Library content and A03 durable Vault membership cross the principal-scoped PostgreSQL boundary during boot repair and ordinary Library moves. Threats: cross-user or cross-account migration, moving content into a foreign/archived Vault, duplicate canonical metadata under concurrent bootstrap, unsafe scaffold deletion, and rollback loss. Controls: one explicit user principal; canonical visible/writable predicates; live same-account destination validation through moveLibraryPage; per-Vault metadata advisory locks; exact owner/account/ID/content/tag/child guards; durable before-state manifest and completion ledger; delete only empty/template marker scaffolding; fail closed on drift. Status: closed in source. Residual risk: the migration is intentionally bounded to Ray's known July 21 shadow-adoption rows and will leave unfamiliar drift untouched for manual review. -->
