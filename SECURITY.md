...
## 11.9 Meeting transport authority repair, July 21, 2026

**SEC-2026-021, confirmed high.** Verified Recall status callbacks and signed participant-audio and visualizer transports carried an opaque meeting session ID, but their first `chatStorage.getSession(...)` read depended on ambient AsyncLocalStorage authority. After the fail-closed principal cure, provider callbacks ran without the meeting owner and returned `Session not found`; status, transcript, and participant audio were accepted by the provider boundary but could not reach the owner-scoped ingest path. Runtime evidence on live session `mrtzy5wnb6821j` showed accepted signed callbacks followed by deterministic lookup failure. The platform Meeting Watchdog also executed under named system principal `timer:meeting-watchdog` and attempted sensitive calendar access across vaults, which the Vault allowlist correctly blocked. Assets: A01/A02/A03; boundaries B03/B06/B13; flows F01/F03/F05. Threats: confused deputy, cross-user disclosure or mutation if transport ownership were weakened, and owner-scoped meeting availability failure under fail-closed enforcement. Controls: IAM-02/03, ING-04, DATA-01, OBS-01. Owner: Integration Owner with Identity and Data Owner. SLA due 2026-07-24.

**Cure implemented, closure evidence pending.** `meeting/owner-principal.ts` now owns the single transport bootstrap boundary: the named, read-only, allowlisted `meeting-transport` principal may resolve an opaque session ID, and every caller immediately transitions to the durable meeting owner principal before user-owned work. Recall ingest, participant audio, and signed visualizer hydration use that boundary. Transcript ingest records its discriminated failure instead of silently discarding it. The Meeting Watchdog and auto-join due-row scanner now enumerate users only at the system layer, then discover, claim, and mutate sensitive calendar rows inside each exact owner principal; calendar metadata claims and outcome writes apply owner predicates at the SQL boundary. Recall's current contract confirms Svix-style status-webhook verification and recommends cryptographic verification for real-time webhook endpoints; those existing signature controls remain required before this identity bridge. Closure requires the production build, merged-main diff, and a post-deploy real meeting showing status/transcript/audio transitions without `Session not found` or `timer:meeting-watchdog` allowlist errors. No active exploit was run and no other user's data was accessed.
