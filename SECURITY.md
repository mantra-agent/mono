...
Owner: Engineering. Closure evidence: merged source and production build. Active authorization testing remains prohibited by repository policy and requires an explicitly scoped synthetic environment. Rollback is the merged PR revert; the additive ledger and invited-subject table may remain unused safely. Residual risk: email ownership can change after claim; grants therefore remain individually revocable before claim, while post-claim access follows the User identity and normal grant revocation path.
