...
- Vault identity is a bounded exception because color carries persistent partition context across surfaces. Every vault picker and renderer must source the canonical near-white tinted family from `VAULT_COLOR_PALETTE` in `shared/models/vaults.ts`; do not create local vault swatches. Full white is the semantic default for Personal; generic new vaults begin on the near-white Sky tint. Persisted custom vault colors remain valid.
