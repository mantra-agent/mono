...
- Physical mobile shell invariant: `AppLayout` owns the top bar, active page or Session Window, and BottomBar as one normal-flow flex column. Do not place keyboard-adjacent mobile controls in independent fixed/sticky layers or reconstruct their layout with a measured spacer; WebKit keyboard dismissal can split visual and hit-test coordinates.
