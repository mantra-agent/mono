...
- Finalized assistant turns preserve streamed content/tool boundaries. During the durable handoff, retain the mounted terminal segments when persisted text and tool chronology are exactly equivalent; only a material persisted change may replace them.
...
- `client/src/components/question-widget.tsx` + `client/src/hooks/use-question-response.ts` — Inline clarification surface. Prompts derive from persisted question tool calls; structured answers travel through the canonical session message endpoint and are rendered back into the originating widget rather than as duplicate user bubbles. Because `await_user` ends the agent run, submission is gated only on live streaming or an in-flight pending turn — never on the run being active. Both selection modes render the standard `SimpleCheckCircle` control; selection mode changes cardinality, not control shape. Sessions with an unanswered question surface in the sidebar's Review group (above Active) via the derived `awaitingQuestionResponse` session field.
