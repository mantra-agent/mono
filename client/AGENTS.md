...
- A displayed stream and its persisted assistant checkpoint are one logical turn. Match ownership by exact `assistantRunId` first, then exact `turnId`, using chronology only for legacy rows without comparable identity; while the live/frozen stream owns the turn, its matching checkpoint must not render a second transcript copy.
...
- Native meeting transcription media is app-owned, not page-owned: `NativeMeetingTranscriptionProvider` owns the browser MediaStream, AudioContext/worklet, authenticated socket, and pull-based microphone level across navigation, while the Meetings action only initiates it from the direct user gesture. Startup is not ready until the server acknowledges one valid PCM frame. The visible live native session reuses the canonical desktop voice-orb surface in `listening` state and keeps the canonical transcript behind the ordinary transcript toggle. Durable meeting lifecycle remains the correctness boundary.
...
The voice orb is a persistent app-shell slot, not a standalone route. `client/src/components/app-shell-immersive.tsx` (`AppShellImmersive`) is the app shell in **immersive-orb presentation mode**: a deliberately lean sibling of the authenticated `AppShell` (in `App.tsx`) that shares the rails + center skeleton but mounts NONE of the heavy authenticated providers (data sync, focus session, executor status, sidebar). It renders for a provisional visitor with no authenticated principal.

- **Single persistent orb slot, above both transports.** The orb is mounted exactly once, inside `LiveVoiceProvider` (`client/src/hooks/use-live-voice.tsx`) and ABOVE both voice providers. `client/src/components/immersive-orb-slot.tsx` (`ImmersiveOrbSlot`) is a PURE visual: it reads `useLiveVoice()` for `visualState` + audio level, owns the entrance settle timer and audio-frame sampling, and renders the single `AgentOrb`. It owns no transport, auth, routing, or rail state. Transport lifecycle is owned by the voice controllers, not the orb.
- **Two transports, one orb, no remount (FR-17).** `AppShellImmersive` is a phase machine (`provisional` → `warming` → `authenticated`). Pre-claim, a provisional `VoiceSessionProvider onboardingToken=...` is the live source. On claim it creates the authenticated FTUE session via the canonical onboarding path (`completeStartupOnboarding` → `/api/onboarding/complete`), mounts a second, chimeless authenticated `VoiceSessionProvider` alongside the provisional one, and warms it in the background. Exactly one transport is the live source at a time; each controller in `client/src/components/immersive-voice.tsx` (`ProvisionalVoiceController` / `AuthenticatedVoiceController`) publishes its `visualState`/audio to the `LiveVoiceProvider` bridge only while active. The provisional transport is torn down (its provider unmounted, silently) ONLY after the authenticated transport has produced audio (`AuthenticatedVoiceController.onProducedAudio`), so the swap has no silence, chime, or flash. Because the orb sits above both providers, none of this remounts it. Never drive the orb from a specific `VoiceSessionProvider` or move it between trees.
- **Rail reveal.** The left Simple rail (`client/src/components/immersive-simple-rail.tsx`, a read-only projection of the provider-free `useHomeFeed()` — the interactive `SimpleFeedView` needs `FocusSessionProvider`, which this shell omits) slides in when the phase reaches `authenticated`. The right rail stays hidden.
- **Chimeless swap.** `VoiceSessionProvider` accepts `suppressChimes`; the shell sets it on both providers during the crossfade so the handoff has no audible chime. The provisional entrance chime still plays once at first connect.
- **Entry contract.** `client/src/lib/immersive-entrance.ts` is the single source of truth for the entrance URL: the provisional voice entrance is `/visualizer?i=<onboardingToken>` (no meeting `token`). `main.tsx` excludes that URL from the standalone visualizer root, and `App.tsx` renders `AppShellImmersive` for it, bypassing `AuthGate`/`BootGate`/`VaultProvider`.
...
- The provisional voice flow (mic prompt, greeting, `POST /api/voice/start`, custom-LLM transport, hash-keyed lease, `VoiceSession.toolMode=none`) is owned by the provisional `VoiceSessionProvider onboardingToken=...`; the authenticated FTUE flow is owned by the sibling authenticated `VoiceSessionProvider` (no token, real principal, `toolMode=standard`). These are the ONLY two voice-start paths in immersive mode — each controller calls `startSession()` exactly once. Do not fork additional voice-start paths or start voice from the orb.
