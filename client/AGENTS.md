...
- Fast Forward is session-local operator mode (`sessionStorage` key `feature-fast-forward:<featureId>`). The shell-mounted `FeatureFastForwardSequencer` owns the walk (same `useSessionLaunch` + `composeFeatureLaunchMessage` path as Play/AI Review) so leaving `/features` does not stop it; FeatureRow owns chrome + Pause→`runStopSession`. Tab-local settle memory lives in `client/src/lib/feature-fast-forward.ts`. Do not add a Feature column or a per-row `useSessionSubscription`.
