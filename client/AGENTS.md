...
- Auto AI Review lives in the same shell host: when a Feature is `needs_review` and Fast Forward is off, launch Review once via the ordinary `runPipelineLaunch("review")` path. No toggle, no stage/status writes. Skip while a pipeline session is humming, on Question/error, or after a Review-fail / launch error for that room (process-local attempt fence cleared when status leaves `needs_review`). Fast Forward mode still owns the walk when on — auto-review must not double-launch.
