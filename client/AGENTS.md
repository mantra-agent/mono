...
- **Single persistent orb slot.** `AppShellImmersive`'s center region always mounts exactly one `client/src/components/immersive-orb-slot.tsx` (`ImmersiveOrbSlot`). `ImmersiveOrbSlot` owns the provisional voice lifecycle (start on mount, entrance settle, audio-level sampling, end on unmount) and renders the single `AgentOrb`. It owns no auth, routing, or rail state.
- **Rails reveal without remount (FR-17).** `railsVisible` is the single discriminant that renders/hides the left/right rail slots around the center orb slot. It defaults to hidden (no principal → orb only, on black). The center slot keeps a fixed position in the tree regardless of `railsVisible`, so the future account-claim step can reveal rails WITHOUT remounting the orb. Never move the orb between trees to toggle rails.
...
- The provisional voice flow (mic prompt, greeting, `POST /api/voice/start`, custom-LLM transport, hash-keyed lease, `VoiceSession.toolMode=none`) is entirely owned by `VoiceSessionProvider onboardingToken=...`. `AppShellImmersive` passes the token through; it must never fork a second voice-start path.
