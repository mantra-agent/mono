...
- Auto AI Review lives in the same shell host: when a Feature is `needs_review` and Fast Forward is off, launch Review once via the ordinary `runPipelineLaunch("review")` path. No toggle, no stage/status writes. The fence is a process-local **room claim** (`claimAutoReviewRoom`) set the moment any path launches Review (auto walker, Fast Forward, or Features row). Auto-review never launches when the claim is set. Clear the claim only when status leaves `needs_review`, or on hard launch error so the button can retry. Do not clear the claim when a session is humming, when Fast Forward turns off, or when settle memory is wiped — that was the double-fire bug. Fast Forward mode still owns the walk when on.
- Review launch requires an authoritative `/api/products` result. Loading or failed Product context reads must delay or fail the launch; they must never become an empty context set, which is reserved for a resolved Product with no configured pages.
