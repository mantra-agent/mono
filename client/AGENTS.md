...
- Session Menu REVIEW is the derived human-review projection. `awaitingReview` is true for an unanswered Question or a principal-visible, Session-linked email draft still in `draft`; `reviewKinds` carries question, ordinary draft, reply, or meeting recap presentation detail. Email send/discard and Question response resolve their owning canonical state and remove the Session on the next event-driven refetch. An active reminder still defers Review to Snooze. Session classification precedence is Archive → Snooze → Live → Review → Active → Pinned → recency buckets.
