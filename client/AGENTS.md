...
- Browser ElevenLabs voice worklets are first-party: `buildBrowserVoiceStartOptions` must pass `workletPaths` and `libsampleratePath` to `/voice/*.worklet.js`. Do not rely on the SDK blob fallback or the jsdelivr libsamplerate CDN; iOS Safari `addModule` stays inside `worker-src` `'self'` only when those paths are present.
