...
- Native meeting transcription media is app-owned, not page-owned: `NativeMeetingTranscriptionProvider` owns the browser MediaStream, AudioContext/worklet, and authenticated socket across navigation, while the Meetings action only initiates it from the direct user gesture. Durable meeting lifecycle remains the correctness boundary.
