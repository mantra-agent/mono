...
- Native meeting transcription media is app-owned, not page-owned: `NativeMeetingTranscriptionProvider` owns the browser MediaStream, AudioContext/worklet, authenticated socket, pull-based microphone level, and native spoken-reply playback across navigation, while the Meetings action only initiates it from the direct user gesture. Construct and request resume of the AudioContext synchronously before the first awaited permission or network operation so iOS WebKit retains that gesture activation. Enabling spoken replies must resume that same context synchronously from the Listen Mode control before the policy request; one abortable owner-scoped audio poll/source stops on mute, leave, capture replacement, or provider unmount. Startup is not ready until the server acknowledges one valid PCM frame. The visible live native session reuses the canonical desktop voice-orb surface in `listening` state and keeps the canonical transcript behind the ordinary transcript toggle. Durable meeting lifecycle remains the correctness boundary.
