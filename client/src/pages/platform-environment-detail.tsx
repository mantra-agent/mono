...
interface SpeechRecognitionBinding {
  id: number;
  environmentId: number;
  connectionId: number;
  provider: string;
  ...
  enabled: boolean;
  sortOrder: number;
  connection: { label: string; status: string; hasCredential: boolean };
}

const SPEECH_ADAPTERS = [
  { adapterKind: "elevenlabs-scribe-realtime", provider: "elevenlabs", label: "ElevenLabs Scribe", model: "scribe_v2_realtime" },
  { adapterKind: "deepgram-realtime", provider: "deepgram", label: "Deepgram Nova-3", model: "nova-3" },
  { adapterKind: "speechmatics-realtime", provider: "speechmatics", label: "Speechmatics", model: "enhanced" },
] as const;

const SPEECH_USE_CASES = [
  { value: "meeting_participant_stream", label: "Participant streams" },
  { value: "meeting_shared_room", label: "Shared room" },
] as const;

type SpeechAdapterKind = (typeof SPEECH_ADAPTERS)[number]["adapterKind"];

function speechAdapterLabel(adapterKind: string): string {
  return SPEECH_ADAPTERS.find((a) => a.adapterKind === adapterKind)?.label ?? adapterKind;
}

function useCaseLabel(useCase: string): string {
  return SPEECH_USE_CASES.find((u) => u.value === useCase)?.label ?? useCase;
}

interface SpeechBindingDraft {
  adapterKind: SpeechAdapterKind;
  connectionId: string;
  useCases: string[];
  enabled: boolean;
  sortOrder: string;
...
function emptySpeechDraft(): SpeechBindingDraft {
  return {
    adapterKind: "speechmatics-realtime",
    connectionId: "",
...
function buildSpeechConfig(draft: SpeechBindingDraft): Record<string, unknown> {
  const base = { version: 1, adapterKind: draft.adapterKind, useCases: draft.useCases };
  if (draft.adapterKind === "speechmatics-realtime") {
    return {
      ...base,
      region: draft.region,
      speakerSensitivity: Math.min(1, Math.max(0, Number(draft.speakerSensitivity) || 0.5)),
      preferCurrentSpeaker: draft.preferCurrentSpeaker,
      maxSpeakers: Math.min(50, Math.max(2, Math.round(Number(draft.maxSpeakers) || 8))),
    };
  }
  return base;
}

function SpeechBindingForm({
  environmentId,
  onDone,
}: {
  environmentId: number;
  ...
  const [draft, setDraft] = useState<SpeechBindingDraft>(emptySpeechDraft);
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const { data: connections } = useQuery<ProviderConnection[]>({ queryKey: ["/api/provider-connections"] });

  const provider = SPEECH_ADAPTERS.find((a) => a.adapterKind === draft.adapterKind)?.provider ?? "speechmatics";
  const providerConnections = (connections || []).filter((c) => c.provider === provider);
  const isSpeechmatics = draft.adapterKind === "speechmatics-realtime";

  const saveMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/platforms/environments/${environmentId}/speech-recognition-bindings`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          connectionId: Number(draft.connectionId),
          provider,
          config: buildSpeechConfig(draft),
          enabled: draft.enabled,
          sortOrder: Math.min(1000, Math.max(0, Math.round(Number(draft.sortOrder) || 0))),
        }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: "Failed" }));
        throw new Error(err.error || "Failed to save speech binding");
      }
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/platforms/environments", environmentId, "speech-recognition-bindings"] });
      toast({ title: "Speech binding saved" });
      onDone();
    },
    onError: (err: Error) => toast({ title: "Error", description: err.message, variant: "destructive" }),
  });
  ...
  const toggleUseCase = (value: string) => {
    setDraft((d) => {
      const has = d.useCases.includes(value);
      const next = has ? d.useCases.filter((u) => u !== value) : [...d.useCases, value];
      return { ...d, useCases: next.length > 0 ? next : d.useCases };
    });
  };

  const canSave = Boolean(draft.connectionId) && draft.useCases.length > 0 && !saveMutation.isPending;

  return (
    <div className="mb-2 space-y-3 rounded-md border border-border/60 bg-muted/30 p-3">
      <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">New speech binding</div>
      <div className="grid gap-2">
        <Label className="text-xs">Provider</Label>
        <Select value={draft.adapterKind} onValueChange={(v) => setDraft((d) => ({ ...d, adapterKind: v as SpeechAdapterKind, connectionId: "" }))}>
          <SelectTrigger className="h-8 text-sm"><SelectValue /></SelectTrigger>
          <SelectContent>
            {SPEECH_ADAPTERS.map((a) => (
              <SelectItem key={a.adapterKind} value={a.adapterKind}>{a.label}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      <div className="grid gap-2">
        <Label className="text-xs">Connection (platform-managed {provider})</Label>
        <Select value={draft.connectionId} onValueChange={(v) => setDraft((d) => ({ ...d, connectionId: v }))}>
      ...
      <div className="grid gap-1.5">
        ...
        <div className="flex flex-wrap gap-2">
          {SPEECH_USE_CASES.map((u) => (
            <Button
              key={u.value}
              type="button"
              variant={draft.useCases.includes(u.value) ? "default" : "outline"}
              size="sm"
              className="h-7 text-xs"
              onClick={() => toggleUseCase(u.value)}
            >
              {u.label}
            </Button>
          ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="grid gap-1.5">
          ...
          <Input value={draft.sortOrder} onChange={(e) => setDraft((d) => ({ ...d, sortOrder: e.target.value }))} className="h-8 text-xs" inputMode="numeric" />
        </div>
        <SwitchRow label="Enabled" checked={draft.enabled} onCheckedChange={(enabled) => setDraft((d) => ({ ...d, enabled }))} />
      </div>
      {isSpeechmatics && (
        <details className="rounded-md border border-border/50 bg-background/60">
          <summary className="cursor-pointer list-none p-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Speechmatics controls</summary>
          <div className="space-y-3 border-t border-border/50 p-2">
            <div className="grid gap-1.5">
              <Label className="text-xs">Region</Label>
              <Select value={draft.region} onValueChange={(v) => setDraft((d) => ({ ...d, region: v as SpeechBindingDraft["region"] }))}>
                <SelectTrigger className="h-8 text-sm"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="us">US (us.rt.speechmatics.com)</SelectItem>
                  <SelectItem value="eu">EU (eu.rt.speechmatics.com)</SelectItem>
                  <SelectItem value="global">Global (routed)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="grid gap-1.5">
                <Label className="text-xs">Speaker sensitivity (0–1)</Label>
                <Input value={draft.speakerSensitivity} onChange={(e) => setDraft((d) => ({ ...d, speakerSensitivity: e.target.value }))} className="h-8 text-xs" inputMode="decimal" />
              </div>
              <div className="grid gap-1.5">
                <Label className="text-xs">Max speakers (≥2)</Label>
                <Input value={draft.maxSpeakers} onChange={(e) => setDraft((d) => ({ ...d, maxSpeakers: e.target.value }))} className="h-8 text-xs" inputMode="numeric" />
              </div>
            </div>
            <SwitchRow label="Prefer current speaker" checked={draft.preferCurrentSpeaker} onCheckedChange={(preferCurrentSpeaker) => setDraft((d) => ({ ...d, preferCurrentSpeaker }))} />
          </div>
        </details>
      )}
      <div className="flex items-center gap-2">
...
function SpeechRecognitionCard({ environmentId }: { environmentId: number }) {
  const [adding, setAdding] = useState(false);
  const [pendingDelete, setPendingDelete] = useState<SpeechRecognitionBinding | null>(null);
  const queryClient = useQueryClient();
  const { toast } = useToast();
  const { data: bindings, isLoading } = useQuery<SpeechRecognitionBinding[]>({
    queryKey: ["/api/platforms/environments", environmentId, "speech-recognition-bindings"],
    enabled: Number.isFinite(environmentId),
  });

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["/api/platforms/environments", environmentId, "speech-recognition-bindings"] });

  const toggleMutation = useMutation({
    mutationFn: async (binding: SpeechRecognitionBinding) => {
      const res = await fetch(`/api/platforms/environments/${environmentId}/speech-recognition-bindings`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: "Failed" }));
        throw new Error(err.error || "Failed to update speech binding");
      }
      return res.json();
    },
  ...
  const removeMutation = useMutation({
    mutationFn: async (bindingId: number) => {
      const res = await fetch(`/api/platforms/environments/${environmentId}/speech-recognition-bindings/${bindingId}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed to remove speech binding");
      return res.json();
    },
    onSuccess: () => { invalidate(); },
    onError: (err: Error) => toast({ title: "Error", description: err.message, variant: "destructive" }),
  ...
  return (
    <div>
      {!adding && (
        ...
        >
          <Plus className="h-3.5 w-3.5 shrink-0" />
          <span>New speech binding</span>
        </button>
      )}
      {adding && <SpeechBindingForm environmentId={environmentId} onDone={() => setAdding(false)} />}
      {isLoading ? (
        <div className="px-2 py-1.5 text-sm text-muted-foreground">Loading speech bindings…</div>
      ) : (bindings || []).length === 0 ? (
        <div className="px-2 py-1.5 text-sm text-muted-foreground">No speech recognition bindings configured.</div>
      ) : (
        (bindings || []).map((binding) => {
          const speechmatics = binding.config.adapterKind === "speechmatics-realtime";
          return (
            <ProfileTreeRow
              key={binding.id}
              label={speechAdapterLabel(binding.adapterKind)}
              icon={<Mic className="h-3.5 w-3.5" />}
              hasValue
              showEmpty
              expandedContent={
                <div className="space-y-2 px-2 py-1.5">
                  <ValueRow label="Model" value={binding.model} mono />
                  <ValueRow label="Use cases" value={binding.useCases.map(useCaseLabel).join(", ") || "—"} />
                  {speechmatics && (
                    <>
                      <ValueRow label="Region" value={String(binding.config.region ?? "us")} />
                      <ValueRow label="Speaker sensitivity" value={String(binding.config.speakerSensitivity ?? 0.5)} />
                      <ValueRow label="Prefer current" value={Boolean(binding.config.preferCurrentSpeaker)} />
                      <ValueRow label="Max speakers" value={String(binding.config.maxSpeakers ?? 8)} />
                    </>
                  )}
                  <ValueRow label="Connection" value={`${binding.connection.label} · ${binding.connection.status}`} />
                  <ValueRow label="Credential" value={binding.connection.hasCredential ? "present" : "missing"} />
                  <ValueRow label="Config fingerprint" value={binding.configFingerprint} mono />
                  <ValueRow label="Priority" value={binding.sortOrder} />
                  <div className="flex items-center justify-end gap-2 pt-1">
                    <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground hover:text-destructive" onClick={() => setPendingDelete(binding)} disabled={removeMutation.isPending}>
            ...
            >
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="text-xs">P{binding.sortOrder}</Badge>
      ...
      <AlertDialog open={pendingDelete !== null} onOpenChange={(open) => !open && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove speech binding?</AlertDialogTitle>
            <AlertDialogDescription>
              This removes the {pendingDelete ? speechAdapterLabel(pendingDelete.adapterKind) : ""} binding from this environment. The provider connection and its credential are not deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
...
function EnvironmentDetailsConfigureCard({ details, environmentId }: { details: EnvironmentDetails; environmentId: number }) {
  return (
    <div className="space-y-1">
      ...
      <EnvironmentSection label="Speech recognition" defaultOpen={false} storageKey={`platform-environment:${environmentId}:section:speech`}>
        <SpeechRecognitionCard environmentId={environmentId} />
      </EnvironmentSection>
